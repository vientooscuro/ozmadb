module OzmaDB.OzmaUtils.Reflection
...
let isNullableType (objectType: Type) =
    ...
    if FSharpType.IsRecord objectType then
        ...
        | mapping when
            not (isRefNull mapping)
            && mapping.SourceConstructFlags.HasFlag(SourceConstructFlags.ObjectType)
            ->
            not (isRefNull <| objectType.GetCustomAttribute<AllowNullLiteralAttribute>())
        | _ ->
            not objectType.IsValueType
            || not (isNull <| Nullable.GetUnderlyingType(objectType))
...
let getOption (map: UnionCase[]) : OptionInfo option =
    let checkNone case =
        if Array.isEmpty case.Fields then Some case.Info else None
...
type NewtypeConverter<'nt>() =
    ...
    override this.CanConvertFrom(context: ITypeDescriptorContext, sourceType: Type) : bool = memberType = sourceType

    override this.CanConvertTo(context: ITypeDescriptorContext, destinationType: Type) : bool =
        typeof<'nt> = destinationType

    override this.ConvertTo
        (context: ITypeDescriptorContext, culture: CultureInfo, value: obj, destinationType: Type)
        : obj =
        convertFrom value

    override this.ConvertFrom(context: ITypeDescriptorContext, culture: CultureInfo, value: obj) : obj = convertTo value
