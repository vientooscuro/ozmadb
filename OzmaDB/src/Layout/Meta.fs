...
let private funExtensions = Set.ofSeq [ SQL.SQLName "pg_trgm" ]
...
let private simplifyIndex: SQL.ValueExpr -> SQL.IndexKey =
    function
...
type private MetaBuilder(layout: Layout) =
    let compileRelatedExpr (expr: ResolvedFieldExpr) : SQL.ValueExpr =
        let (arguments, ret) =
            compileSingleFieldExpr defaultCompilationFlags layout relatedCompilationFlags emptyArguments expr
    ...
    let makeIndexColumnMeta
        ...
        let opClass =
            col.OpClass
            |> Option.map (fun opClass -> Map.find opClass allowedOpClasses |> Map.find index.Type)
    ...
    let makeIndexMeta (ref: ResolvedEntityRef) (entity: ResolvedEntity) (index: ResolvedIndex) : SQL.IndexMeta =
        ...
        let includedColumns =
            Array.map (compileRelatedExpr >> simplifyIndex) index.IncludedExpressions
    ...
    let makeColumnFieldMeta
        (ref: ResolvedFieldRef)
        (entity: ResolvedEntity)
        (field: ResolvedColumnField)
        : SQL.ColumnMeta * (SQL.ConstraintName * (SQL.MigrationKeysSet * SQL.ConstraintMeta)) seq =
        ...
        let res =
            { DataType =
                SQL.mapValueType (fun (x: SQL.SimpleType) -> x.ToSQLRawString()) (compileFieldType field.FieldType)
        ...
        let constr =
            ...
            | FTScalar(SFTReference(entityRef, opts)) ->
                ...
                let constrKey =
                    sprintf "__foreign__%O__%O__%O" ref.Entity.Schema ref.Entity.Name ref.Name
            ...
            | FTScalar(SFTEnum vals) ->
                ...
                let constrKey =
                    sprintf "__enum__%O__%O__%O" ref.Entity.Schema ref.Entity.Name ref.Name

                let constrName =
                    SQL.SQLName <| sprintf "__enum__%s__%s" entity.HashName field.HashName
    ...
    let makeMaterializedComputedFieldMeta
        (ref: ResolvedFieldRef)
        (entity: ResolvedEntity)
        ...
        let columnType =
            if exprIsLocal rootInfo.Flags then
                ...
                let (arguments, compiled) =
                    compileSingleFieldExpr
                        defaultCompilationFlags
                        layout
                        materializedCompilationFlags
                        emptyArguments
                        resolved
    ...
    let rec getPathReferences
        (fieldRef: ResolvedFieldRef)
        (relatedFields: PathReferencesMap)
        (fields: (ResolvedEntityRef * FieldName) list)
        : PathReferencesMap =
        match fields with
        | [] -> relatedFields
        | ((refEntityRef, refName) :: refs) ->
            ...
            let (fieldName, field) =
                ...
                | Some { Field = RColumnField col
                         Name = fieldName } -> (fieldName, col)
                | _ -> failwith "Unexpected non-column field"

            let relatedFields =
                Map.add { fieldRef with Name = fieldName } (entity, field) relatedFields
    ...
    let makeCheckConstraintMeta
        (entityRef: ResolvedEntityRef)
        (entity: ResolvedEntity)
        (modifyExpr: SQL.ValueExpr -> SQL.ValueExpr)
        (name: ConstraintName)
        (constr: ResolvedCheckConstraint)
        : (SQL.SQLName * (SQL.MigrationKeysSet * SQL.ConstraintMeta)) seq =
    ...
    let makeEntityMeta
        (entityRef: ResolvedEntityRef)
        (entity: ResolvedEntity)
        : SQL.TableObjectsMeta * (SQL.SQLName * SQL.RelationMeta) seq =
        let tableName = compileResolvedEntityRef entity.Root

        let makeEntityCheckConstraint modifyExpr (name, constr) =
            makeCheckConstraintMeta entityRef entity modifyExpr name constr

        let makeMaterializedComputedField (name, maybeField) =
            match maybeField with
            | Ok field ->
                if
                    not field.IsMaterialized
                    || Option.isNone field.Root
                    || Option.isSome field.InheritedFrom
                then
                    None
                else
        ...
        let materializedFieldColumns =
            entity.ComputedFields |> Map.toSeq |> Seq.mapMaybe makeMaterializedComputedField
        ...
        let (tableObjects, extraObjects) =
            ...
            | None ->
                    ...
                    if not <| hasSubType entity then
                                ...
                                SQL.CTPlain
                                    { DefaultExpr =
                                        if entity.IsAbstract then
                                            ...
                                            Some(SQL.VEValue <| SQL.VString entity.TypeName) } }
                            : SQL.ColumnMeta
                        ...
                        let constrs =
                            ...
                            | Some checkExpr ->
                                ...
                                Seq.singleton (
                                    typeCheckName,
                                    (Set.singleton typeCheckKey, SQL.CMCheck(String.comparable checkExpr))
                                )
                        ...
                        let indexes =
                            Seq.singleton (typeIndexName, (Set.singleton typeIndexKey, subEntityIndex))
                ...
                let makeColumn (name, field: ResolvedColumnField) =
                    ...
                    let keys =
                        if
                            entityRef = { Schema = OzmaQLName "public"
                                          Name = OzmaQLName "events" }
                            && name = OzmaQLName "row_id"
                        then
                ...
                let columnObjects =
                    entity.ColumnFields |> Map.toSeq |> Seq.map makeColumn |> Seq.cache

                let userColumns =
                    columnObjects
                    |> Seq.map (fun (name, keys, column, constrs) -> (name, (keys, column)))

                let columnConstraints =
                    columnObjects |> Seq.collect (fun (name, keys, column, constrs) -> constrs)

                let checkConstraintObjects =
                    entity.CheckConstraints
                    |> Map.toSeq
                    |> Seq.collect (makeEntityCheckConstraint id)
                ...
                let constraints =
                    Seq.concat
                        [ idConstraints
                          ...
                          columnConstraints
                          checkConstraintObjects ]
                    |> Map.ofSeqUnique

                let columns =
                    Seq.concat [ idColumns; subEntityColumns; userColumns; materializedFieldColumns ]
                    |> Map.ofSeq
                ...
                let indexObjects =
                    Seq.map (fun (name, (keys, index)) -> (name, (SQL.OMIndex(keys, tableName.Name, index)))) allIndexes
            ...
            | Some parent ->
                    ...
                    if Option.isSome field.InheritedFrom then
                        ...
                        let (meta, extraConstrs) =
                            ...
                            | Some check ->
                                ...
                                let notnullName =
                                    SQL.SQLName <| sprintf "__notnull__%s__%s" entity.HashName field.HashName

                                let notnullKey =
                                    sprintf "__notnull__%O__%O__%O" entityRef.Schema entityRef.Name name

                                let extraConstrs =
                                    Seq.singleton (
                                        notnullName,
                                        (Set.singleton notnullKey, SQL.CMCheck(String.comparable expr))
                                    )
                ...
                let columnObjects =
                    entity.ColumnFields |> Map.toSeq |> Seq.mapMaybe makeColumn |> Seq.cache
                ...
                let checkConstraintObjects =
                    entity.CheckConstraints
                    |> Map.toSeq
                    |> Seq.collect (makeEntityCheckConstraint modify)

                let constraints =
                    Seq.concat [ checkConstraintObjects; columnConstraints ] |> Map.ofSeqUnique
        ...
        let uniqueConstraints =
            entity.UniqueConstraints |> Map.toSeq |> Seq.map makeUniqueConstraint
        ...
        let makeReferenceIndex (name, field: ResolvedColumnField) =
            match field with
            | { InheritedFrom = None
                FieldType = FTScalar(SFTReference(refEntityRef, opts)) } ->
                ...
                let sqlName =
                    SQL.SQLName <| sprintf "__refindex__%s__%s" entity.HashName field.HashName
        ...
        let tableObjects =
            { tableObjects with
                Constraints =
                    Seq.fold
                        (fun constrs (name, value) -> Map.addUnique name value constrs)
                        tableObjects.Constraints
                        uniqueConstraints }
    ...
    let makeSchemaMeta
        ...
        : Map<ResolvedEntityRef, SQL.TableObjectsMeta> * SQL.SchemaMeta =
        ...
        let tables =
            ret
            |> Seq.map (fun (root, (table, objects)) -> (root, table))
            |> Map.ofSeqWith (fun name -> SQL.unionTableObjectsMeta)

        let objects =
            ret |> Seq.collect (fun (root, (table, objects)) -> objects) |> Map.ofSeq
    ...
    let buildLayoutMeta (subLayout: Layout) : SQL.DatabaseMeta =
        ...
        let schemas =
            ret
            |> Seq.map (fun (name, (tables, objects)) -> (name, (Set.empty, objects)))
            |> Map.ofSeq
        ...
        let tables =
            ret
            |> Seq.map (fun (name, (tables, objects)) -> tables)
            |> Seq.fold (Map.unionWith SQL.unionTableObjectsMeta) Map.empty

        let splitTables =
            tables
            |> Map.toSeq
            |> Seq.map makeSplitTable
        ...
        let mergeTables (schemaName: SQL.SQLName) (keys: SQL.MigrationKeysSet, schema: SQL.SchemaMeta) =
            ...
            | Some extras ->
                let schema =
                    { schema with
                        Relations = Map.union schema.Relations extras }
        ...
        let schemas =
            schemas
            |> Map.filter (fun name (keys, meta) -> not <| SQL.schemaMetaIsEmpty meta)
