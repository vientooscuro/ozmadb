...
type ChunkException(message: string, innerException: exn, isUserException: bool) =
    ...
    new(message: string, innerException: exn) = ChunkException(message, innerException, isUserException innerException)

    new(message: string) = ChunkException(message, null, true)

type SourceChunkArgument = { Type: string; Value: JToken }
...
let private maybeAddAnonymousInt
    (int: int option)
    (argValues: LocalArgumentsMap)
    (arguments: QueryArguments)
    : LocalArgumentsMap * QueryArguments * SQL.ValueExpr option =
    match int with
...
let private compileWhereExpr
    (layout: Layout)
    (initialArguments: QueryArguments)
    (chunkArguments: Map<ArgumentName, ArgumentName>)
    ...
    let compileExpr (args: QueryArguments) =
        let (info, expr) =
            compileSingleFieldExpr defaultCompilationFlags layout emptyExprCompilationFlags args expr

        (info.Arguments, expr)

    modifyArgumentsInNamespace convertArgument compileExpr initialArguments

let private genericResolveWhere
    (layout: Layout)
    (columns: ChunkColumnsMap)
    (chunk: SourceChunkWhere)
    : ResolvedChunkWhere =
    let parsedExpr =
    ...
    let parsedArguments =
        Map.map (fun name -> parseChunkArgument) chunk.Arguments |> OrderedMap.ofMap
    ...
    let (info, resolvedExpr) =
        try
            resolveSingleFieldExpr
                callbacks
                parsedArguments
                emptyExprResolutionFlags
                (SFCustom customMapping)
                parsedExpr
        with :? QueryResolveException as e ->
            raisefWithInner ChunkException e ""
...
let private searchToWhere (columns: ChunkColumnsMap) (search: string) : ResolvedChunkWhere =
    ...
    let getColumnSearchOp (KeyValue(name: FieldName, info: ChunkColumn)) =
        let columnRef =
            lazy (({ Entity = None; Name = name }: FieldRef) |> VRColumn |> resolvedRefFieldExpr)
        ...
        | SQL.VTScalar SQL.STInt ->
            Some(fun (word: SearchWord) ->
                ...
                | None -> None
                | Some arg -> Some <| FEBinaryOp(columnRef.Value, BOEq, arg))
        | _ -> None
    ...
    let optimized = words |> Seq.map appearsInColumn |> andFieldExprs
...
let genericResolveChunk (layout: Layout) (columns: ChunkColumnsMap) (chunk: SourceQueryChunk) : ResolvedQueryChunk =
    let searchExpr = Option.map (searchToWhere columns) chunk.Search
    let where = Option.map (genericResolveWhere layout columns) chunk.Where
    let searchExpr = Option.unionWith unionResolvedChunkWhere searchExpr where
...
let queryExprChunk
    ...
    : LocalArgumentsMap * Query<SQL.SelectExpr> =
