module OzmaDB.OzmaQL.UsedReferences
...
type private UsedReferencesBuilder(layout: ILayoutBits) =
    ...
    let rec addField (extra: ObjectMap) (ref: ResolvedFieldRef) (field: ResolvedFieldInfo) =
        ...
        | RId
        | RSubEntity
        | RColumnField _ -> usedDatabase <- addUsedFieldRef ref usedFieldSelect usedDatabase
        | RComputedField comp ->
    ...
    and buildForPath
        ...
        (asRoot: bool)
        : (ResolvedEntityRef * PathArrow) list -> unit =
        function
        | [] ->
    ...
    and buildForReference (ref: LinkedBoundFieldRef) =
        let info =
            ObjectMap.tryFindType<FieldRefMeta> ref.Extra
        ...
        | None -> assert (Array.isEmpty ref.Ref.Path)

    and buildForResult: ResolvedQueryResult -> unit =
        function
        | QRAll alias -> ()
    ...
    and buildForBoundAttributeExpr: ResolvedBoundAttributeExpr -> unit =
        function
    ...
    and buildForAttribute (attr: ResolvedAttribute) = buildForFieldExpr attr.Expression
    ...
    and buildForOrderColumn (ord: ResolvedOrderColumn) = buildForFieldExpr ord.Expr |> ignore
    ...
    and buildForSelectTreeExpr: ResolvedSelectTreeExpr -> unit =
        function
    ...
    and buildForCommonTableExpr (cte: ResolvedCommonTableExpr) = buildForSelectExpr cte.Expr
    ...
    and buildForFromTableExpr (expr: ResolvedFromTableExpr) = buildForTableExpr expr.Expression

    and buildForFromExpr: ResolvedFromExpr -> unit =
        function
        | FEntity fromEnt ->
    ...
    and buildForUpdateAssignExpr (entityRef: ResolvedEntityRef) =
        function
        | UAESet(name, expr) ->
    ...
    let flattenUsedEntity (entityRef: ResolvedEntityRef) (usedEntity: UsedEntity) =
        ...
        let makeFlatField (fieldName, usedField: UsedField) =
            ...
            | RColumnField col ->
                ...
                Some(
                    fieldEntity,
                    { emptyUsedEntity with
                        Fields = Map.singleton fieldName newUsedField }
                )
        ...
        let makeFlatDeleteField (fieldName, field: ResolvedColumnField) =
            ...
            (fieldEntity,
             { emptyUsedEntity with
                 Fields = Map.singleton fieldName usedFieldSelect })

        let fieldChildren =
            usedEntity.Fields
            |> Map.toSeq
            |> Seq.mapMaybe makeFlatField
        ...
        let children =
            unionFlatUsedChildren (unionFlatUsedChildren thisChildren fieldChildren) deleteChildren
...
let singleKnownFlatEntity
    (rootRef: ResolvedEntityRef)
    (entityRef: ResolvedEntityRef)
    (usedEntity: UsedEntity)
    (fields: (ResolvedFieldRef * UsedField) seq)
    : FlatUsedDatabase =
    let getField (ref: ResolvedFieldRef, usedField) =
        if ref.Name = funId || ref.Name = funSubEntity then
            ...
            Some(
                ref.Entity,
                { emptyUsedEntity with
                    Fields = Map.singleton ref.Name usedField }
            )

    let flatFieldEntities =
        fields |> Seq.mapMaybe getField |> Map.ofSeqWith (fun name -> unionUsedEntities)
