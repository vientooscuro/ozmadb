...
type QueryResolveException(message: string, innerException: exn, isUserException: bool) =
    inherit UserException(message, innerException, isUserException)

    new(message: string, innerException: exn) =
        QueryResolveException(message, innerException, isUserException innerException)

    new(message: string) = QueryResolveException(message, null, true)

let private allowedPragmas: Set<PragmaName> =
    Set.ofList
        [ OzmaQLName "enable_bitmapscan"
          OzmaQLName "enable_gathermerge"
          OzmaQLName "enable_hashagg"
          OzmaQLName "enable_hashjoin"
          OzmaQLName "enable_indexscan"
          OzmaQLName "enable_indexonlyscan"
          OzmaQLName "enable_material"
          OzmaQLName "enable_mergejoin"
          OzmaQLName "enable_nestloop"
          OzmaQLName "enable_parallel_append"
          OzmaQLName "enable_parallel_hash"
          OzmaQLName "enable_partition_pruning"
          OzmaQLName "enable_partitionwise_join"
          OzmaQLName "enable_partitionwise_aggregate"
          OzmaQLName "enable_seqscan"
...
let compileScalarType: ScalarFieldType<_> -> SQL.SimpleType =
    function
    | SFTInt -> SQL.STInt
...
let compileFieldType: FieldType<_> -> SQL.SimpleValueType =
    function
    | FTScalar stype -> SQL.VTScalar <| compileScalarType stype
    | FTArray stype -> SQL.VTArray <| compileScalarType stype

let decompileScalarType: SQL.SimpleType -> ScalarFieldType<_> =
    function
    | SQL.STInt -> SFTInt
...
let decompileFieldType: SQL.SimpleValueType -> FieldType<_> =
    function
...
let isScalarValueSubtype
    (layout: ILayoutBits)
    (wanted: ResolvedScalarFieldType)
    (given: ResolvedScalarFieldType)
    : bool =
...
type FromFieldKey =
    { FromId: FromId; Path: FieldName list }
...
[<NoEquality; NoComparison>]
type private BoundArgument = { Ref: ArgumentRef; Immediate: bool }
...
let private getAmbiguousMapping k =
    function
    | FVAmbiguous set -> set
    | FVResolved info -> Set.singleton k

let private unionNameMappingValue
    (k: 'k)
    (a: NameMappingValue<'k, 'v>)
    (b: NameMappingValue<'k, 'v>)
    : NameMappingValue<'k, 'v> =
...
let customEntityMapping (ref: ResolvedEntityRef) =
    { Ref = ref; ForceSQLTable = None }: CustomEntityMapping
...
let private explodeFieldRef (fieldRef: FieldRef) : FieldRef seq =
    seq {
        ...
        | None -> ()
        | Some { Schema = maybeSchema
                 Name = entityName } ->
            yield
                ({ Entity = Some { Schema = None; Name = entityName }
                   Name = fieldName }
                : FieldRef)
...
let private explodeResolvedFieldRef (fieldRef: ResolvedFieldRef) : FieldRef seq =
    seq {
        let ({ Entity = { Schema = schemaName
                          Name = entityName }
               Name = fieldName }: ResolvedFieldRef) =
            fieldRef
...
let private boundValueToMapping: BoundValue option -> ResolvedFieldMapping =
    function
    | None -> FMUnbound
    | Some bound -> FMBound bound

let private resolvedFieldForcedSQLName
    : GenericResolvedField<ResolvedColumnField, 'comp> -> SQL.ColumnName option when 'comp :> IComputedFieldBits =
    function
    | RId -> Some sqlFunId
...
let private boundValueForcedSQLName: BoundValue -> SQL.ColumnName option =
    function
    | BVColumn { Field = field
                 Header = { Immediate = true; Single = false } } -> resolvedFieldForcedSQLName field
    | _ -> None

let private fieldsToFieldMapping
    (fromEntityId: FromEntityId)
    (maybeEntityRef: EntityRef option)
    (forceSQLTable: SQL.TableRef option)
    (fields: SubqueryFieldsMap)
    : FieldMapping =
...
let private typeRestrictedFieldsToFieldMapping
    (layout: ILayoutBits)
    (fromEntityId: FromEntityId)
    (flags: FromMappingFlags)
    (maybeEntityRef: EntityRef option)
    (parentRef: ResolvedEntityRef)
    (children: ResolvedEntityRef seq)
    : FieldMapping =
    ...
    let mapEntity (entityRef: ResolvedEntityRef) =
        ...
        let expandName
            (name: FieldName, field: ResolvedFieldBits)
            : (FieldRef * NameMappingValue<FieldRef, FieldMappingInfo>) seq =
            let key =
            ...
            explodeResolvedFieldRef { Entity = parentRef; Name = name }
            |> Seq.map (fun ref -> (ref, resolved))

        entity.Fields |> Seq.filter filterField |> Seq.collect expandName

    children |> Seq.collect mapEntity |> Map.ofSeq

let private customToFieldMapping
    (layout: ILayoutBits)
    (fromEntityId: FromEntityId)
    (isInner: bool)
    (mapping: CustomFromMapping)
    : FieldMapping =
...
let private mapRecursiveValue (f: 'a -> 'b) =
    function
    | RValue a -> RValue(f a)
...
let private renameFields (names: FieldName[]) (fields: SubqueryFields) : SubqueryFields =
    if Array.length names <> Array.length fields then
        raisef QueryResolveException "Inconsistent number of columns"
...
let private renameFieldsInSubquery (names: FieldName[]) (info: SubqueryInfo) : SubqueryInfo =
    { info with
        Fields = renameFields names info.Fields }

let private getFieldsMap (fields: SubqueryFields) : SubqueryFieldsMap =
    fields
    |> Seq.mapMaybe (fun (mname, info) -> Option.map (fun name -> (name, info)) mname)
    |> Map.ofSeq
...
let private renderOzmaQLArguments (arguments: ResolvedArgumentsMap) : string =
    if OrderedMap.isEmpty arguments then
        ...
        let printArgument (name: ArgumentRef, arg: ResolvedArgument) =
            match name with
            | PGlobal _ -> None
            | PLocal _ -> Some <| sprintf "%s %s" (name.ToOzmaQLString()) (arg.ToOzmaQLString())

        arguments
        |> OrderedMap.toSeq
        |> Seq.mapMaybe printArgument
        |> String.concat ", "
        |> sprintf "(%s):"
...
type ResolvedViewExpr =
    ...
    member this.ToOzmaQLString() =
        String.concatWithWhitespaces
            [ renderOzmaQLArguments this.Arguments
              this.Select.ToOzmaQLString()
              optionToOzmaQLString this.MainEntity ]
...
type ResolvedCommandExpr =
    ...
    member this.ToOzmaQLString() =
        String.concatWithWhitespaces [ renderOzmaQLArguments this.Arguments; this.Command.ToOzmaQLString() ]
...
let private unionSubqueryInfo (a: SubqueryInfo) (b: SubqueryInfo) =
    if Array.length a.Fields <> Array.length b.Fields then
        raisef QueryResolveException "Different number of columns in a set operation expression"

    let newFields =
        Array.map2 (fun (name1, info1) (name2, info2) -> (name1, unionSubqueryField info1 info2)) a.Fields b.Fields
...
let private checkName (OzmaQLName name) : unit =
    if not (goodName name) || String.length name > SQL.sqlIdentifierLength then
        raisef QueryResolveException "Invalid name: %s" name

let private selfSchema: SchemaName = OzmaQLName "__self"

let tryResolveEntityRef
    (homeSchema: SchemaName option)
    (forceSchema: bool)
    (ref: EntityRef)
    : ResolvedEntityRef option =
...
let private resolveEntityRef (homeSchema: SchemaName option) (ref: EntityRef) : ResolvedEntityRef =
    ...
    | Some newRef -> newRef
    | None -> raisef QueryResolveException "Unspecified schema in reference: %O" ref

let resolveScalarFieldType
    (layout: IEntitiesSet)
    (homeSchema: SchemaName option)
    (allowReferenceOptions: bool)
    : ParsedScalarFieldType -> ResolvedScalarFieldType =
    function
    | SFTInt -> SFTInt
    ...
    | SFTReference(entityRef, mopts) ->
        match mopts with
        | Some opts when not allowReferenceOptions ->
            raisef QueryResolveException "Reference delete options are not allowed here"
        | _ -> ()

        let resolvedRef = resolveEntityRef homeSchema entityRef

        if not <| layout.HasVisibleEntity resolvedRef then
            raisef QueryResolveException "Cannot find entity %O from reference type" resolvedRef

        SFTReference(resolvedRef, mopts)
    | SFTEnum vals ->
        if OrderedSet.isEmpty vals then
            raisef QueryResolveException "Enums must not be empty"
...
let resolveFieldType
    ...
    (homeSchema: SchemaName option)
    (allowReferenceOptions: bool)
    : ParsedFieldType -> ResolvedFieldType =
    function
...
let relaxScalarFieldType: ResolvedScalarFieldType -> ParsedScalarFieldType =
    function
    | SFTInt -> SFTInt
    | SFTDecimal -> SFTDecimal
    | SFTString -> SFTString
...
let relaxFieldType: ResolvedFieldType -> ParsedFieldType =
    function
    | FTArray typ -> FTArray <| relaxScalarFieldType typ
    | FTScalar typ -> FTScalar <| relaxScalarFieldType typ

let getResolvedScalarFieldType: ParsedScalarFieldType -> ResolvedScalarFieldType =
    function
    | SFTInt -> SFTInt
    | SFTDecimal -> SFTDecimal
    | SFTString -> SFTString
...
let getResolvedFieldType: ParsedFieldType -> ResolvedFieldType =
    function
    | FTArray typ -> FTArray <| getResolvedScalarFieldType typ
    | FTScalar typ -> FTScalar <| getResolvedScalarFieldType typ

let resolveCastScalarFieldType: ScalarFieldType<_> -> ScalarFieldType<_> =
    function
    | SFTInt -> SFTInt
    | SFTDecimal -> SFTDecimal
    | SFTString -> SFTString
    ...
    | SFTUserViewRef -> SFTUserViewRef
    | SFTUuid -> SFTUuid
    | SFTReference(entityRef, opts) -> raisef QueryResolveException "Cannot cast to reference type"
    | SFTEnum vals -> raisef QueryResolveException "Cannot cast to enum type"

let resolveCastFieldType: FieldType<_> -> FieldType<_> =
    function
    | FTArray typ -> FTArray <| resolveCastScalarFieldType typ
    | FTScalar typ -> FTScalar <| resolveCastScalarFieldType typ

let getGlobalArgument =
    function
...
type ResolvedSingleSelectMeta = { HasAggregates: bool }
...
let private boundValueMeta: BoundValue -> BoundValueMeta =
    function
...
type private ResolvedCommonTableExprTempMeta = { mutable MainEntity: bool }

type private ResolvedCommonTableExprsTempMeta =
    { Bindings: ResolvedCommonTableExprsMap }

type ResolvedCommonTableExprInfo = { MainEntity: bool }

let private cteBindings (select: ResolvedSelectExpr) : ResolvedCommonTableExprsMap =
    match select.CTEs with
    | Some ctes -> (ObjectMap.findType<ResolvedCommonTableExprsTempMeta> ctes.Extra).Bindings
    | None -> Map.empty
...
let rec private tryFindMainEntityFromExpr
    (ctes: ResolvedCommonTableExprsMap)
    (currentCte: EntityName option)
    : ResolvedFromExpr -> RecursiveValue<ResolvedEntityRef> option =
    function
    | FEntity { Ref = { Schema = Some schemaName
                        Name = name } } -> Some <| RValue { Schema = schemaName; Name = name }
    | FEntity { Ref = { Schema = None; Name = name } } ->
        ...
        if extra.MainEntity then
            ...
            tryFindMainEntityExpr ctes (Some name) cte.Expr
...
and private tryFindMainEntityTableExpr
    (ctes: ResolvedCommonTableExprsMap)
    (currentCte: EntityName option)
    : ResolvedTableExpr -> RecursiveValue<ResolvedEntityRef> option =
    function
    | TESelect sel ->
...
and private tryFindMainEntityTreeExpr
    (ctes: ResolvedCommonTableExprsMap)
    (currentCte: EntityName option)
    : ResolvedSelectTreeExpr -> RecursiveValue<MainEntityResult> option =
    function
    | SSelect({ From = Some from } as sel) ->
        ...
        if info.HasAggregates || not <| Array.isEmpty sel.GroupBy then
            ...
            let refResults ref = { Entity = ref; Results = sel.Results }

            tryFindMainEntityFromExpr ctes currentCte from
            |> Option.map (mapRecursiveValue refResults)
    | SSelect _ -> None
    | SValues _ -> None
    | SSetOp setOp ->
        monad' {
...
and private tryFindMainEntityExpr
    (ctes: ResolvedCommonTableExprsMap)
    (currentCte: EntityName option)
    (select: ResolvedSelectExpr)
    : RecursiveValue<MainEntityResult> option =
...
type private FromEntityIdMeta = { Id: FromEntityId; IsInner: bool }

let private failCte (name: EntityName) =
    raisef QueryResolveException "Table %O not found" name
...
let rec private foldCommonAncestor
    (layout: ILayoutBits)
    (baseRef: ResolvedEntityRef)
    (refA: ResolvedEntityRef)
    (entityA: IEntityBits)
    (refB: ResolvedEntityRef)
    (entityB: IEntityBits)
    : ResolvedEntityRef * IEntityBits =
...
let private findCommonAncestor
    (layout: ILayoutBits)
    (baseRef: ResolvedEntityRef)
    (entities: ResolvedEntityRef seq)
    : (ResolvedEntityRef * IEntityBits) option =
...
let private findFieldWithoutContext
    (layout: ILayoutBits)
    (entityRef: ResolvedEntityRef)
    (fieldName: FieldName)
    : (ResolvedEntityRef * IEntityBits * ResolvedFieldInfo) option =
...
let private findFieldWithContext
    (layout: ILayoutBits)
    (typeCtxs: TypeContextsMap)
    (key: FromFieldKey)
    (parentEntity: ResolvedEntityRef)
    (fieldName: FieldName)
    : (ResolvedEntityRef * IEntityBits * ResolvedFieldInfo) option =
...
let private resolveSubEntity
    (layout: ILayoutBits)
    (outerTypeCtxs: TypeContextsMap)
    (ctx: SubEntityContext)
    (field: LinkedBoundFieldRef)
    (subEntityInfo: SubEntityRef)
    : TypeContextsMap * SubEntityRef =
    let (fieldRef, typeCtxKey) =
        ...
        | VRColumn col ->
            ...
            let fieldRef =
                if Array.isEmpty field.Ref.Path then
                    ...
                    | _ -> raisef QueryResolveException "Unbound field in a type assertion"
                else
        ...
        | VRArgument arg ->
            if Array.isEmpty field.Ref.Path then
                raisef QueryResolveException "Unbound field in a type assertion"
    ...
    | _ -> raisef QueryResolveException "Bound field in a type assertion is not a 'sub_entity' field: %O" field

    let subEntityRef =
        { Schema = Option.defaultValue fieldRef.Entity.Schema subEntityInfo.Ref.Schema
          Name = subEntityInfo.Ref.Name }

    let subEntity =
        match layout.FindEntity subEntityRef with
        | None -> raisef QueryResolveException "Couldn't find subentity %O" subEntityInfo.Ref
        | Some r -> r
    ...
    let neededTypes =
        ...
        | SECInheritedFrom ->
            if checkInheritance layout subEntityRef fieldRef.Entity then
                ...
                allPossibleEntities layout subEntityRef
                |> mapPossibleEntities (Seq.map fst >> Set.ofSeq)
            else
                raisef QueryResolveException "Entities in a type assertion are not in the same hierarchy"
        | SECOfType ->
            if subEntity.IsAbstract then
                raisef QueryResolveException "Instances of abstract entity %O do not exist" subEntityRef
    ...
    let ret =
        { Ref = relaxEntityRef subEntityRef
...
let replaceFieldRefInExpr
    (updateEntityRef: LinkedBoundFieldRef -> LinkedBoundFieldRef)
    : ResolvedFieldExpr -> ResolvedFieldExpr =
    let resolveReference: LinkedBoundFieldRef -> LinkedBoundFieldRef =
        function
        | { Ref = { Ref = VRColumn col } } as ref -> updateEntityRef ref
        | ref -> ref

    let mapper =
        { idFieldExprMapper with
            FieldReference = resolveReference }
...
let replaceEntityRefInField (localRef: EntityRef option) (ref: LinkedBoundFieldRef) : LinkedBoundFieldRef =
    ...
    | VRColumn col ->
        { ref with
            Ref =
                { ref.Ref with
                    Ref = VRColumn { col with Entity = localRef } } }
    | _ -> failwith "Not a column reference"

let replaceEntityRefInExpr (localRef: EntityRef option) =
    replaceFieldRefInExpr (replaceEntityRefInField localRef)
...
let replacePathInField
    (layout: ILayoutBits)
    (sourceRef: LinkedBoundFieldRef)
    (replaceLastPrefixArrow: bool)
    ...
    let newPath =
        Seq.concat [ pathPrefix; Seq.singleton newFieldArrow; Array.toSeq ref.Ref.Path ]
        |> Array.ofSeq
    ...
    let newBoundPath =
        Seq.concat [ boundPrefix; Seq.singleton newBoundEntity; destMeta.Path ]
        |> Array.ofSeq
    ...
    { Ref =
        { Ref = sourceRef.Ref.Ref
          Path = newPath
          AsRoot = ref.Ref.AsRoot }
      Extra = newExtra }

let replacePathInExpr (layout: ILayoutBits) (sourceRef: LinkedBoundFieldRef) (replaceLastPrefixArrow: bool) =
    replaceFieldRefInExpr (replacePathInField layout sourceRef replaceLastPrefixArrow)
    ...
    | Some meta ->
        let filterCase (case: VirtualFieldCase) =
            ...
            if Set.isEmpty possibleCases then
                ...
                Some
                    { case with
                        PossibleEntities = possibleCases }

        cases |> Seq.mapMaybe filterCase

let computedFieldCases
    (layout: ILayoutBits)
    (extra: ObjectMap)
    (ref: ResolvedFieldRef)
    (comp: ResolvedComputedField)
    : (VirtualFieldCase * ResolvedComputedField) seq =
...
let private mainEntityColumnsMeta
    (fields: SubqueryFieldsMap)
    (ref: ResolvedEntityRef)
    (columns: ResolvedQueryResult[])
    : MainEntityMeta =
    let getField: ResolvedQueryResult -> (FieldName * FieldName) option =
        function
        ...
        | QRExpr result ->
            ...
            | Some { Bound = Some(BVColumn { Ref = fieldRef
                                             Field = RColumnField _ }) } when fieldRef.Entity = ref ->
                Some(name, fieldRef.Name)
            | _ -> None
...
let private resolveMainEntity
    (layout: ILayoutBits)
    (homeSchema: SchemaName option)
    (fields: SubqueryFieldsMap)
    (query: ResolvedSelectExpr)
    (main: ParsedMainEntity)
    : ResolvedMainEntity option =
    match tryFindMainEntity query with
    | None -> raisef QueryResolveException "Main entity not matched in the expression"
    | Some found ->
        let requestedRef = resolveEntityRef homeSchema main.Entity

        if found.Entity <> requestedRef then
            raisef QueryResolveException "Main entity %O is specified, but %O is detected" requestedRef ref
        ...
        if main.ForInsert then
            ...
            if entity.IsAbstract then
                raisef QueryResolveException "Entity is abstract: %O" main.Entity
                ...
                if Option.isNone field.DefaultValue && not field.IsNullable then
                    if not (Map.containsKey fieldName meta.ColumnsToFields) then
                        raisef
                            ...
                            "Required inserted entity field is not in the view expression: %O"
                            fieldName
...
let enumDomainValuesColumn = OzmaQLName "value"
...
let private unionFromExprInfo (a: FromExprInfo) (b: FromExprInfo) =
    let entities =
        ...
        with Failure msg ->
            raisef QueryResolveException "Clashing entity names: %s" msg
...
let rec private relabelFromExprType (typeContexts: TypeContextsMap) =
    function
    | FEntity entity ->
        ...
        FEntity
            { entity with
                Extra = ObjectMap.singleton ctxMeta }
...
type private QueryResolver(callbacks: ResolveCallbacks, findArgument: FindArgument, resolveFlags: ExprResolutionFlags) =
    ...
    let getArgument arg =
        ...
        | None -> raisef QueryResolveException "Unknown argument: %O" arg
    ...
    let notTypeContext (ctx: TypeContext) : PossibleEntities<TypeContext> =
        if Set.isEmpty ctx.AllowedSubtypes then
            ...
            let allEntities =
                allPossibleEntitiesList layout ctx.Type |> Seq.map fst |> Set.ofSeq
    ...
    let orTypeContext (ctxA: TypeContext) (ctxB: TypeContext) : TypeContext option =
        ...
        let allEntities =
            allPossibleEntitiesList layout ctxA.Type |> Seq.map fst |> Set.ofSeq
    ...
    let findEntityByRef (entityRef: ResolvedEntityRef) (allowHidden: bool) : IEntityBits =
        ...
        | _ -> raisef QueryResolveException "Entity not found: %O" ref

    let createFromMapping
        (fromEntityId: FromEntityId)
        (entityRef: ResolvedEntityRef)
        (alias: (EntityRef option) option)
        (flags: FromMappingFlags)
        : FieldMapping =
        let entity = findEntityByRef entityRef flags.AllowHidden

        if flags.WithoutChildren && entity.IsAbstract then
            raisef QueryResolveException "Abstract entity cannot be selected as ONLY"
        ...
        let mapping =
            fieldsToFieldMapping fromEntityId mappingRef flags.ForceSQLTable fields

        if flags.WithoutChildren then
            mapping
        else
            let extraMapping =
                typeRestrictedFieldsToFieldMapping
                    layout
                    fromEntityId
                    flags
                    mappingRef
                    entityRef
                    (entity.Children |> Map.keys)

            Map.unionUnique extraMapping mapping

    let domainFromInfo
        (fromEntityId: FromEntityId)
        (boundValue: BoundValue option)
        (fieldType: ResolvedFieldType)
        (tableName: EntityName)
        (flags: DomainExprInfo)
        : FromExprInfo =
        if flags.AsRoot then
            if not resolveFlags.Privileged then
                raisef QueryResolveException "Cannot specify roles in non-privileged user views"
        ...
        let fields =
            ...
            | typ -> raisef QueryResolveException "Field %O doesn't have a domain" typ
    ...
    let resolvePath
        (typeCtxs: TypeContextsMap)
        (firstHeader: UnresolvedBoundColumn)
        (fullPath: PathArrow list)
        : BoundColumn list =
        ...
        let rec traverse (boundField: BoundColumn) : PathArrow list -> BoundColumn list =
            function
            | [] -> [ boundField ]
            | (ref :: refs) ->
                if ref.AsRoot then
                ...
                | _ -> raisef QueryResolveException "Invalid dereference: %O" ref

        traverse (getBoundField firstHeader) fullPath

    let resolvePathByType
        (typeCtxs: TypeContextsMap)
        (fromId: FromId)
        (fieldType: ResolvedFieldType)
        (path: PathArrow[])
        ...
        let parentEntity =
            ...
            | _ -> raisef QueryResolveException "Value is not a reference"
    ...
    let resolveReference
        ...
        (typeCtxs: TypeContextsMap)
        (f: LinkedFieldRef)
        : ReferenceInfo * LinkedBoundFieldRef =
        if f.AsRoot then
            if not resolveFlags.Privileged then
                raisef QueryResolveException "Cannot specify roles in non-privileged user views"
        ...
        let (refInfo, newRef) =
            match f.Ref with
            | VRColumn ref ->
                let rec findInMappings =
                    function
                    | [] -> raisef QueryResolveException "Unknown field: %O" ref
                    | mapping :: mappings ->
                        match Map.tryFind ref mapping with
                        | Some(FVResolved info) -> info
                        | Some(FVAmbiguous entities) ->
                            raisef QueryResolveException "Ambiguous reference %O. Possible values: %O" ref entities
                ...
                if not (Array.isEmpty f.Path) && isExternal then
                    raisef QueryResolveException "Arrows for external fields are currently not supported"

                let getFromCurrentBoundField bound =
                    let (outer, remaining) =
                        resolvePath typeCtxs bound (Array.toList f.Path) |> List.snoc
                ...
                let (outerValue, innerValue, boundFields) =
                    ...
                    | FMBound(BVArgument arg) ->
                        if Array.isEmpty f.Path then
                            ...
                            let path =
                                resolvePathByType typeCtxs (FIEntity info.EntityId) argInfo.ArgType f.Path
                    ...
                    | FMUnbound ->
                        if Array.isEmpty f.Path then
                            ...
                            let fieldType =
                                ...
                                | Some typ -> typ
                                | None -> raisef QueryResolveException "Dereference of an unbound field in %O" f
                ...
                let newRef =
                    { Ref = { f with Ref = VRColumn newFieldRef }
                      Extra = extra }

                let exprInfo =
                    if isExternal then
                        { emptyResolvedExprInfo with
                            ExternalEntities = Set.singleton info.EntityId }
                    else
            ...
            | VRArgument argRef ->
                if ctx.NoArguments then
                    raisef QueryResolveException "Arguments are not allowed here: %O" argRef

                let argInfo = getArgument argRef
                let hasSubquery = not <| Array.isEmpty f.Path
                let boundMeta = { Ref = argRef; Immediate = true }: BoundArgumentMeta
                ...
                let (innerValue, boundInfo) =
                    if not hasSubquery then
                        ...
                        let innerBound = { Ref = argRef; Immediate = true }: BoundArgument
                        (BVArgument innerBound, ObjectMap.singleton boundInfo)
                    else
                        let boundFields =
                            resolvePathByType typeCtxs (FIArgument argRef) argInfo.ArgType f.Path
                ...
                let newRef =
                    { Ref = { f with Ref = VRArgument argRef }
                      Extra = boundInfo }
        ...
        let refInfo =
            if not <| Array.isEmpty f.Path then
                { refInfo with
                    ExprInfo =
                        { refInfo.ExprInfo with
                            Flags =
                                { refInfo.ExprInfo.Flags with
                                    HasArrows = true } } }
            else
                refInfo
    ...
    let resolveLimitFieldExpr (expr: ParsedFieldExpr) : ResolvedFieldExpr =
        let resolveRef: LinkedFieldRef -> LinkedBoundFieldRef =
            function
            | { Ref = VRArgument name; Path = [||] } as ref ->
                let argInfo = getArgument name
                { Ref = ref; Extra = ObjectMap.empty }
            | ref -> raisef QueryResolveException "Invalid reference in LIMIT or OFFSET: %O" ref

        let voidSubEntity field subEntity =
            raisef QueryResolveException "Forbidden type assertion in LIMIT or OFFSET"
        ...
        with :? UnexpectedExprException as e ->
            raisefWithInner QueryResolveException e ""
    ...
    let resolveUserViewRef (ref: UserViewRef) : UserViewRef =
        ...
        if not <| callbacks.HasUserView fullRef then
            raisef QueryResolveException "Referenced user view not found: %O" fullRef
    ...
    let fromEntityMapping (ctx: Context) (isInner: bool) (entity: ParsedFromEntity) : FromEntityMappingInfo =
        ...
        let getEntityInfo schemaName entityName =
            ...
            let mapping =
                createFromMapping fromEntityId resRef (Option.map Some punRef) mappingFlags
        ...
        let (mapping, types, resolvedRef, name, exprInfo) =
            ...
            | { Schema = Some schemaName
                Name = entityName } -> getEntityInfo schemaName entityName
            | { Schema = None; Name = entityName } ->
                match ctx.ResolveCTE entityName with
                | Some cteInfo ->
                    if entity.AsRoot then
                        raisef QueryResolveException "Roles can only be specified for entities"

                    if entity.Only then
                        raisef QueryResolveException "ONLY can only be specified for entities"

                    let newName = Option.defaultValue entityName entity.Alias
                    let mappingRef = { Schema = None; Name = newName }: EntityRef

                    let mapping =
                        fieldsToFieldMapping fromEntityId (Some mappingRef) None (getFieldsMap cteInfo.Fields)
                ...
                | None ->
                    ...
                    | None -> raisef QueryResolveException "Entity not found: %O" entity.Ref
    ...
    and resolveResultColumn
        (flags: SelectFlags)
        (ctx: Context)
        (result: ParsedQueryColumnResult)
        : ResultInfo * ResolvedQueryColumnResult =
        ...
        let (attrRefs, attributes) =
            resolveBoundAttributesMap ctx resultInfo.ExprInfo result.Attributes
        ...
        let resultInfo =
            { resultInfo with
                ExprInfo = unionResolvedExprInfo resultInfo.ExprInfo attrRefs }

        (resultInfo, ret)

    // Should be in sync with resultField
    and resolveResultExpr
        (flags: SelectFlags)
        (ctx: Context)
        (name: FieldName option)
        : ParsedFieldExpr -> ResultInfo * ResolvedFieldExpr =
        function
        | FERef f ->
            ...
            let (resultBound, innerType) =
                ...
                | Some(BVColumn field) ->
                    if field.ForceRename && Option.isNone name && flags.RequireNames then
                        raisef
                            QueryResolveException
                            "Field should be explicitly named in result expression: %s"
                            (f.ToOzmaQLString())
        ...
        | e ->
            match name with
            | Some n -> checkName n
            | None when flags.RequireNames ->
                raisef QueryResolveException "Unnamed results are allowed only inside expression queries"
            | None -> ()
    ...
    and resolveAttributeExpr (ctx: Context) (expr: ParsedFieldExpr) : ResolvedExprInfo * ResolvedFieldExpr =
        ...
        if info.Info.Flags.HasAggregates then
            raisef QueryResolveException "Aggregate functions are not allowed here"
    ...
    and resolveBoundAttribute
        (ctx: Context)
        (boundExprInfo: ResolvedExprInfo)
        (attr: ParsedBoundAttribute)
        : ResolvedExprInfo * ResolvedBoundAttribute =
        ...
        | BAExpr expr ->
            ...
            (info,
             { Internal = newAttr.Internal
               Expression = BAExpr newAttr.Expression
               Dependency = newAttr.Dependency })
        | BAMapping mapping ->
            (boundExprInfo,
             { Internal = attr.Internal
               Expression = BAMapping mapping
               Dependency = exprDependency boundExprInfo })
        | BAArrayMapping mapping ->
            (boundExprInfo,
             { Internal = attr.Internal
               Expression = BAArrayMapping mapping
               Dependency = exprDependency boundExprInfo })

    and resolveAttributesMap
        (ctx: Context)
        (attributes: ParsedAttributesMap)
        : ResolvedExprInfo * ResolvedAttributesMap =
    ...
    and resolveBoundAttributesMap
        (ctx: Context)
        (boundExprInfo: ResolvedExprInfo)
        (attributes: ParsedBoundAttributesMap)
        : ResolvedExprInfo * ResolvedBoundAttributesMap =
        let mutable exprInfo = emptyResolvedExprInfo
    ...
    and resolveEntityReference (ctx: Context) (entityRef: EntityRef) : FromEntityInfo * EntityRef =
        ...
        | None -> raisef QueryResolveException "Entity %O not found" entityRef
        | Some entityInfo ->
            match entityRef.Schema with
            | Some _ as schema when entityInfo.Schema <> schema ->
                raisef QueryResolveException "Entity %O not found" entityRef
            | _ -> ()
    ...
    and resolveFieldExpr (ctx: Context) (expr: ParsedFieldExpr) : InnerResolvedExprInfo * ResolvedFieldExpr =
        ...
        let resolveQuery query =
            let (selectInfo, res) =
                resolveSelectExpr (subSelectContext ctx) subExprSelectFlags query
            // We only consider an expression to have references if it's for a local entity.
            let localEntities =
                Set.intersect selectInfo.ExprInfo.ExternalEntities ctx.LocalEntities
            // We add entities that are not local to our own external entities.
            let externalEntities =
                Set.difference selectInfo.ExprInfo.ExternalEntities ctx.LocalEntities
        ...
        let rec traverse (outerTypeCtxs: TypeContexts) =
            function
            | FEValue value ->
            ...
            | FEEntityAttr(eref, attr) ->
                ...
                if not <| Set.contains attr entityInfo.EntityAttributes then
                    raisef QueryResolveException "Entity attribute %O is not specified for %O" attr eref
                ...
                exprInfo <-
                    { exprInfo with
                        Flags = unionResolvedExprFlags exprInfo.Flags unknownResolvedExprFlags }
            ...
            | FEFieldAttr(fref, attr) ->
                ...
                if not <| fieldAttributeExists refInfo attr then
                    raisef QueryResolveException "Field attribute %O is not specified for field %O" attr newRef
                ...
                exprInfo <-
                    { exprInfo with
                        Flags = unionResolvedExprFlags exprInfo.Flags newFlags }
            ...
            | FEAnd(a, b) ->
                ...
                let (innerTypeCtxs2, newB) =
                    traverse (andTypeContexts outerTypeCtxs innerTypeCtxs1) b
            ...
            | FEInheritedFrom(f, nam) ->
                ...
                let (innerTypeCtxs, newNam) =
                    resolveSubEntity layout outerTypeCtxs.Map SECInheritedFrom newF nam
            ...
            | FEOfType(f, nam) ->
                ...
                let (innerTypeCtxs, newNam) =
                    resolveSubEntity layout outerTypeCtxs.Map SECOfType newF nam
    ...
    and resolveOrderLimitClause
        (ctx: Context)
        (limits: ParsedOrderLimitClause)
        : ResolvedExprInfo * ResolvedOrderLimitClause =
        let mutable exprInfo = emptyResolvedExprInfo
        ...
        let resolveOrderBy (ord: ParsedOrderColumn) =
            ...
            if info.Info.Flags.HasAggregates then
                raisef QueryResolveException "Aggregates are not allowed here"
    ...
    and resolveCommonTableExpr
        (ctx: Context)
        (flags: SelectFlags)
        (allowRecursive: bool)
        (name: EntityName)
        (cte: ParsedCommonTableExpr)
        : SubqueryInfo * ResolvedCommonTableExpr =
        ...
        let (results, tree) =
            ...
            | SSetOp setOp when allowRecursive ->
                ...
                let (results2, b') =
                    resolveSelectExpr newCtx { cteFlags with RequireNames = false } setOp.B
        ...
        let results =
            { results with
                ExprInfo = unionSubqueryExprInfo results.ExprInfo exprInfo }
        ...
        let info = { MainEntity = false }: ResolvedCommonTableExprTempMeta
    ...
    and resolveCommonTableExprs
        (ctx: Context)
        (flags: SelectFlags)
        (ctes: ParsedCommonTableExprs)
        : CTEsInfo * ResolvedCommonTableExprs =
        ...
        let exprsMap =
            ...
            with Failure msg ->
                raisef QueryResolveException "Clashing names in WITH binding: %s" msg

        let mutable resultsMap: Map<EntityName, SubqueryInfo * ResolvedCommonTableExpr> =
            Map.empty
            ...
            if not (Map.containsKey name resultsMap) then
                ...
                let rec tmpResolveCte (currName: EntityName) =
                    ...
                    | None when ctes.Recursive ->
                        ...
                        | Some expr when name = currName -> raisef QueryResolveException "Invalid recursive statement"
                        | Some expr when Set.contains currName processing ->
                            raisef QueryResolveException "Mutual recursion in WITH statement"
                        | Some cte ->
        ...
        let meta =
            { Bindings = Map.map (fun name (results, cte) -> cte) resultsMap }: ResolvedCommonTableExprsTempMeta
    ...
    and resolveSelectExpr
        (ctx: Context)
        (flags: SelectFlags)
        (select: ParsedSelectExpr)
        : SubqueryInfo * ResolvedSelectExpr =
        ...
        let results =
            { results with
                ExprInfo = unionSubqueryExprInfo results.ExprInfo ctesInfo }

        (results, newSelect)

    and finishResolveSetOpExpr
        (ctx: Context)
        (setOp: ParsedSetOperationExpr)
        (results1: SubqueryInfo)
        (a: ResolvedSelectExpr)
        (results2: SubqueryInfo)
        (b: ResolvedSelectExpr)
        (flags: SelectFlags)
        : SubqueryInfo * ResolvedSelectTreeExpr =
        if flags.RequireNames then
            for (name, info) in results1.Fields do
                if Option.isNone name then
                    raisef QueryResolveException "Name is required for column"
        ...
        let orderLimitMapping =
            getFieldsMap newResults.Fields |> Map.toSeq |> Seq.map mapField |> Map.ofSeq

        let orderLimitCtx =
            { ctx with
                FieldMaps = orderLimitMapping :: ctx.FieldMaps }

        let (limitsInfo, resolvedLimits) =
            resolveOrderLimitClause orderLimitCtx setOp.OrderLimit

        if limitsInfo.Flags.HasArrows then
            raisef
                QueryResolveException
                "Dereferences are not allowed in ORDER BY clauses for set expressions: %O"
                resolvedLimits.OrderBy

        let newResults =
            { newResults with
                ExprInfo = unionSubqueryExprInfo newResults.ExprInfo (resolvedToSubqueryExprInfo limitsInfo) }
    ...
    and resolveSelectTreeExpr
        (ctx: Context)
        (flags: SelectFlags)
        : ParsedSelectTreeExpr -> SubqueryInfo * ResolvedSelectTreeExpr =
        function
        | SSelect query ->
        ...
        | SValues values ->
            if flags.RequireNames then
                raisef QueryResolveException "Column aliases are required for this VALUES entry"
            ...
            let mapValue =
                function
                | VVDefault -> raisef QueryResolveException "DEFAULT is not allowed here"
                | VVExpr expr ->
                    let (info, newExpr) = resolveFieldExpr ctx expr
            ...
            let mapEntry entry =
                if Array.length entry <> valuesLength then
                    raisef QueryResolveException "Invalid number of items in VALUES entry: %i" (Array.length entry)
            ...
            let fields =
                Seq.init valuesLength (fun i -> (None, emptySubqueryField)) |> Array.ofSeq
        ...
        | SSetOp setOp ->
            ...
            let (results2, b') =
                resolveSelectExpr ctx { flags with RequireNames = false } setOp.B

            finishResolveSetOpExpr ctx setOp results1 a' results2 b' flags

    and resolveSingleSelectExpr
        (ctx: Context)
        (flags: SelectFlags)
        ...
        let hasAggregates =
            ...
            || not (Array.isEmpty qGroupBy)

        let results = Array.map snd rawResults

        let getFields: ResultInfo * ResolvedQueryResult -> FieldName option * SubqueryField =
            function
        ...
        with Failure msg ->
            raisef QueryResolveException "Clashing result names: %s" msg

        let extra = { HasAggregates = hasAggregates }: ResolvedSingleSelectMeta
    ...
    and resolveFromExpr
        (ctx: Context)
        (fromInfo: FromExprInfo)
        (isInner: bool)
        (flags: SelectFlags)
        : ParsedFromExpr -> FromExprInfo * ResolvedFromExpr =
        function
        | FEntity entity ->
            if ctx.NoFetches then
                raisef QueryResolveException "Fetches are not allowed here"
        ...
        | FTableExpr tab ->
            let localCtx = if tab.Lateral then addFromToContext fromInfo ctx else ctx
            let fromEntityId = nextFromEntityId ()
            ...
            let (fromInfo, newExpr) =
                ...
                | TESelect sel ->
                    ...
                    let (info, newQ) =
                        resolveSelectExpr
                            (subSelectContext localCtx)
                            { flags with
                                RequireNames = Option.isNone tab.Alias.Fields }
                            sel
                ...
                | TEDomain(fieldRef, flags) ->
                    ...
                    let fieldType =
                        match info.Type with
                        | Some typ -> typ
                        | _ -> raisef QueryResolveException "Field %O has unknown type" fieldRef

                    if Option.isSome tab.Alias.Fields then
                        raisef QueryResolveException "Can't rename fields from domain"

                    let myFromInfo =
                        domainFromInfo fromEntityId info.InnerValue fieldType tab.Alias.Name flags
                ...
                | TEFieldDomain(entityRef, fieldName, flags) ->
                    ...
                    let field =
                        ...
                        | None -> raisef QueryResolveException "Field doesn't exist in %O: %O" resRef fieldName
                        | Some f -> f
                    ...
                    let fieldType =
                        ...
                        | _ ->
                            raisef
                                QueryResolveException
                                "Field %O has unknown type"
                                { Entity = resRef; Name = fieldName }
                    ...
                    let myFromInfo =
                        domainFromInfo fromEntityId (Some <| BVColumn bound) fieldType tab.Alias.Name flags
            ...
            let (infoB, newB) =
                ...
                with :? QueryResolveException as e when join.Type = Cross && e.Message.StartsWith("Unknown field:") ->
                    ...
                    | FTableExpr tab when not tab.Lateral ->
                        ...
                        resolveFromExpr ctx infoA isInner2 flags (FTableExpr lateralTab)
                    | _ -> reraise ()
    ...
    and resolveInsertValue (ctx: Context) : ParsedValuesValue -> ResolvedExprInfo * ResolvedValuesValue =
        function
        | VVDefault -> (emptyResolvedExprInfo, VVDefault)
        | VVExpr expr ->
            let (info, newExpr) = resolveFieldExpr ctx expr
            (info.Info, VVExpr newExpr)

    and resolveInsertExpr
        (ctx: Context)
        (flags: SelectFlags)
        (insert: ParsedInsertExpr)
        : SubqueryInfo * ResolvedInsertExpr =
        let mutable exprInfo = emptySubqueryExprInfo
        ...
        if entity.IsAbstract then
            raisef QueryResolveException "Entity %O is abstract" entityRef

        let fields =
            try
                Set.ofSeqUnique insert.Fields
            with Failure err ->
                raisef QueryResolveException "Clashing insert field: %s" err

        for fieldName in insert.Fields do
            match entity.FindField fieldName with
            | Some { ForceRename = false
                     Field = RColumnField field } -> ()
            | _ -> raisef QueryResolveException "Field not found: %O" { Entity = entityRef; Name = fieldName }

        for (fieldName, field) in entity.ColumnFields do
            if not (fieldIsOptional field) && not (Set.contains fieldName fields) then
                raisef QueryResolveException "Required field not provided: %O" fieldName
        ...
        let resolveInsertValues vals =
            if Array.length vals <> Array.length insert.Fields then
                raisef QueryResolveException "Number of values is not consistent"
        ...
        let source =
            ...
            | ISSelect select ->
                ...
                if Array.length selInfo.Fields <> Array.length insert.Fields then
                    raisef
                        QueryResolveException
                        "Queries in INSERT must have the same number of columns as number of inserted columns"
    ...
    and resolveUpdateExpr
        (ctx: Context)
        (flags: SelectFlags)
        (update: ParsedUpdateExpr)
        : SubqueryInfo * ResolvedUpdateExpr =
        let mutable exprInfo = emptySubqueryExprInfo
        ...
        let (ctx, newCtes) =
            ...
            | Some ctes ->
                ...
                let ctx =
                    { ctx with
                        ResolveCTE = ctesInfo.ResolveCTE }
        ...
        let (entityInfo, from) =
            ...
            | Some from ->
                let (newEntityInfo, newFrom) =
                    resolveFromExpr ctx entityFromInfo.SingleFrom true flags from
        ...
        let checkField fieldName =
            ...
            | Some { ForceRename = false
                ...
                if field.IsImmutable then
                    raisef QueryResolveException "Field %O is immutable" fieldName
            | _ -> raisef QueryResolveException "Field not found: %O" { Entity = entityRef; Name = fieldName }

            if Set.contains fieldName columns then
                raisef QueryResolveException "Field %O is assigned multiple times" fieldName

            columns <- Set.add fieldName columns

        let resolveFieldUpdate =
            function
            ...
            | UAESelect(cols, select) ->
                ...
                if Array.length selectInfo.Fields <> Array.length cols then
                    raisef
                        QueryResolveException
                        "Queries in UPDATE must have the same number of columns as number of updated columns"
    ...
    and resolveDeleteExpr
        (ctx: Context)
        (flags: SelectFlags)
        (delete: ParsedDeleteExpr)
        : SubqueryInfo * ResolvedDeleteExpr =
        let mutable exprInfo = emptySubqueryExprInfo
    ...
    and resolveDataExpr
        (ctx: Context)
        (flags: SelectFlags)
        (dataExpr: ParsedDataExpr)
        : SubqueryInfo * ResolvedDataExpr =
        match dataExpr with
        | DESelect select ->
    ...
    member this.ResolveSelectExpr flags select =
        resolveSelectExpr emptyContext flags select

    member this.ResolveDataExpr flags dataExpr =
        resolveDataExpr emptyContext flags dataExpr
    ...
    member this.ResolveEntityAttributesMap
        (entityRef: ResolvedEntityRef)
        (attrs: ParsedBoundAttributesMap)
        : ResolvedBoundAttributesMap =
        let flags =
            { IsInner = true
              AllowHidden = true
              WithoutChildren = false
...
and private relabelValuesValue =
    function
    | VVDefault -> VVDefault
...
and private relabelUpdateAssignExpr =
    function
...
and private relabelSelectTreeExpr: ResolvedSelectTreeExpr -> ResolvedSelectTreeExpr =
    function
...
and private relabelBoundAttributeExpr: ResolvedBoundAttributeExpr -> ResolvedBoundAttributeExpr =
    function
...
and private relabelFromExpr: ResolvedFromExpr -> ResolvedFromExpr =
    function
...
and private relabelQueryResult: ResolvedQueryResult -> ResolvedQueryResult =
    function
...
let private resolveArgument (callbacks: ResolveCallbacks) (arg: ParsedArgument) : ResolvedArgument =
    let argType =
        resolveFieldType callbacks.Layout callbacks.HomeSchema false arg.ArgType

    match arg.DefaultValue with
    | None -> ()
    | Some def ->
        match def with
        | FNull when not arg.Optional -> raisef QueryResolveException "Invalid default value, expected %O" argType
        | _ -> ()

        if not (isValueOfSubtype argType def) then
            raisef QueryResolveException "Invalid default value, expected %O" argType
...
let private resolveArgumentsMap
    (callbacks: ResolveCallbacks)
    (rawArguments: ParsedArgumentsMap)
    (flags: ExprResolutionFlags)
    : ResolvedArgumentsMap * FindArgument =
    ...
    let attrsQualifier =
        QueryResolver(callbacks, getHalfArgument, emptyExprResolutionFlags)
...
let private getArgumentsQualifier
    (callbacks: ResolveCallbacks)
...
let resolveSelectExpr
    (callbacks: ResolveCallbacks)
    (arguments: ParsedArgumentsMap)
    (flags: ExprResolutionFlags)
    (select: ParsedSelectExpr)
    : ResolvedArgumentsMap * ResolvedSelectExpr =
    ...
    if Array.length info.Fields <> 1 then
        raisef QueryResolveException "Expression queries must have only one resulting column"
...
let resolveSingleFieldExpr
    (callbacks: ResolveCallbacks)
    (arguments: ParsedArgumentsMap)
    (flags: ExprResolutionFlags)
    (fromMapping: SingleFromMapping)
    (expr: ParsedFieldExpr)
    : SingleFieldExprInfo * ResolvedFieldExpr =
    let (localArguments, qualifier) = getArgumentsQualifier callbacks arguments flags
...
let resolveEntityAttributesMap
    (callbacks: ResolveCallbacks)
    (flags: ExprResolutionFlags)
    (entityRef: ResolvedEntityRef)
    (attrs: ParsedBoundAttributesMap)
...
let resolveViewExpr
    (callbacks: ResolveCallbacks)
    (flags: ExprResolutionFlags)
    (viewExpr: ParsedViewExpr)
    : ResolvedViewExpr =
    let (localArguments, qualifier) =
        getArgumentsQualifier callbacks viewExpr.Arguments flags
...
let resolveCommandExpr
    (callbacks: ResolveCallbacks)
    (flags: ExprResolutionFlags)
    (cmdExpr: ParsedCommandExpr)
    : ResolvedCommandExpr =
    let (localArguments, qualifier) =
        getArgumentsQualifier callbacks cmdExpr.Arguments flags

    let (results, qCommand) =
        qualifier.ResolveDataExpr viewExprSelectFlags cmdExpr.Command
