module OzmaDB.Operations.Entity
...
type EntityOperationError =
    | [<CaseKey(null, Type = CaseSerialization.InnerObject)>] EOEExecution of OzmaQLExecutionError
    ...
    | [<CaseKey(null, Type = CaseSerialization.InnerObject, IgnoreFields = [| "Id" |])>] EOEWithId of
        Id: RowId *
        Inner: EntityOperationError
    ...
    member this.Details =
        ...
        | EOEWithId(id, inner) -> inner.Details |> Map.add "id" (JToken.op_Implicit id)
        | _ -> Map.empty
...
let private runQuery
    ...
    task {
        try
            ...
            return!
                runFunc
                    (prefix + query.Expression.ToSQLString())
                    (prepareArguments query.Arguments placeholders)
                    cancellationToken
...
let private runOptionalIntQuery
...
let private runOptionalValuesQuery
    (connection: QueryConnection)
    query
    maybeLayout
    comments
...
let private runIntsQuery (connection: QueryConnection) maybeLayout query comments placeholders cancellationToken =
    let execute query args cancellationToken =
        connection.ExecuteColumnValuesQuery query args cancellationToken
        <| fun name typ rows -> task { return! rows.Select(SQL.parseIntValue).ToArrayAsync(cancellationToken) }

    runQuery execute maybeLayout query comments placeholders cancellationToken

let private databaseArg = PLocal(OzmaQLName "database")

let private databaseSizeQuery: Lazy<Query<SQL.SelectExpr>> =
    lazy
        (let (nameArg, arguments) =
            addArgument databaseArg (requiredArgument <| FTScalar SFTString) emptyArguments

         let dbSizeExpr =
             SQL.VEFunc(SQL.SQLName "pg_database_size", [| SQL.VEPlaceholder nameArg.PlaceholderId |])
...
let getEntityInfo
    ...
    | None ->
        ...
        let entityTriggers =
            Option.defaultValue emptyMergedTriggersEntity (triggers.FindEntity entityRef)

        serializeEntity layout entityTriggers entity
    | Some role ->
        try
            serializeEntityRestricted layout triggers role entityRef
        with :? PermissionsEntityException as e when isUserException e ->
...
let private countAndThrow
    ...
    task {
        if Option.isNone applyRole then
            ...
            | Some id ->
                raise
                <| EntityOperationException(EOEWithId(int id, EOEAccessDenied "The entry is not accessible"))
    }
    ...
    task {
        let! rowInfo =
            task {
                ...
                | _ ->
                    match!
                        runOptionalValuesQuery connection (Some layout) query comments argumentValues cancellationToken
                    with
                    | None -> return None
                    | Some values ->
                        let findValue wantedName =
                            let (name, typ, value) =
                                values |> Seq.find (fun (name, typ, value) -> name = wantedName)
        ...
        | None ->
            ...
            do!
                countAndThrow
                    ...
                    applyRole
                    tableRef
                    whereExprWithoutRole
                    argumentsWithoutRole
                    argumentValues
                    cancellationToken
...
let private altKeyCheck
    ...
    let getCheck (fieldName: FieldName) =
        ...
        | Some arg ->
            ...
            let (argInfo, newArguments) =
                addArgument argName (requiredArgument field.FieldType) arguments

            arguments <- newArguments
            argumentValues <- Map.add argName arg argumentValues

            let usedField =
                { Entity = Option.defaultValue entityRef field.InheritedFrom
                  Name = fieldName }
            ...
            SQL.VEBinaryOp(
                ...
                SQL.BOEq,
                SQL.VEPlaceholder argInfo.PlaceholderId
            )
            ...
            SQL.VEBinaryOp(
                SQL.VEColumn
                    { Table = Some aliasRef
                      Name = sqlFunId },
                SQL.BOEq,
                SQL.VEPlaceholder idArg.PlaceholderId
            )
...
let private buildSelectOperation
    ...
    let results =
        bits.Columns |> Seq.map (fun expr -> SQL.SCExpr(None, expr)) |> Seq.toArray
    ...
    let query =
        ...
        | Some role ->
            let usedDatabase =
                singleKnownFlatEntity entity.Root entityRef usedEntitySelect bits.UsedFields
...
let private runCheckExpr
    ...
    task {
        ...
        let idCol =
            ...
            <| VRColumn
                { Entity = Some <| relaxEntityRef entityRef
                  Name = funId }
            : ResolvedFieldExpr
        ...
        | Some(name, typ, SQL.VBool false) ->
            raise
            <| EntityOperationException(EOEAccessDenied "Access to the entries is denied")
...
let resolveAltKey
    ...
    task {
        ...
        let opQuery =
            buildSelectOperation layout applyRole entityRef
            <| fun aliasRef ->
                ...
                { Where = alt.Where
                  ...
                  UsedFields =
                    Seq.append alt.UsedFields (Seq.singleton ({ Entity = entityRef; Name = funId }, usedFieldSelect))
        ...
        | None ->
            do!
                countAndThrow
                    ...
                    applyRole
                    opQuery.Table
                    opQuery.WhereWithoutRole
                    opQuery.ArgumentsWithoutRole
                    argumentValues
                    cancellationToken
...
let resolveKey
    ...
    task {
        ...
        | RKAlt(name, args) ->
            return! resolveAltKey connection globalArgs layout applyRole entityRef comments name args cancellationToken
    }
    ...
    task {
        ...
        let (subEntityColumn, subEntityValue) =
            if hasSubType entity then
                let (argInfo, argName, newArguments) =
                    addAnonymousArgument (requiredArgument <| FTScalar SFTString) arguments

                arguments <- newArguments
                argumentValues <- Map.add (PLocal argName) (FString entity.TypeName) argumentValues

                (Seq.singleton (null, sqlFunSubEntity),
                 Seq.singleton (argInfo.PlaceholderId |> SQL.VEPlaceholder |> SQL.IVExpr))
            else
                (Seq.empty, Seq.empty)
        ...
        let insertColumns =
            Seq.append idColumn (Seq.append subEntityColumn dataColumns) |> Seq.toArray
        ...
        let getRow (rowArgs: LocalArgumentsMap) =
            let getValue (fieldName: FieldName, field: ResolvedColumnField) =
                ...
                | Some arg ->
                    let (argInfo, argName, newArguments) =
                        addAnonymousArgument (requiredArgument field.FieldType) arguments
        ...
        | Some role ->
            ...
            let usedDatabase =
                singleKnownFlatEntity entity.Root entityRef usedEntityInsert usedFields
...
let updateEntry
    ...
    task {
        ...
        let mutable argumentValues =
            Map.unionUnique (Map.mapKeys PGlobal globalArgs) keyCheck.ArgumentValues
        ...
        let getValue (fieldName: FieldName, fieldValue: FieldValue) =
            ...
            let argument =
                { requiredArgument field.FieldType with
        ...
        let query =
            ...
            | Some role ->
                ...
                let usedFields =
                    updateArgs |> Map.keys |> Seq.map getUsedField |> Seq.append keyCheck.UsedFields

                let usedDatabase =
                    singleKnownFlatEntity entity.Root entityRef usedEntityUpdate usedFields
    ...
    task {
                    ...
                    SQL.VEBinaryOp(
                        SQL.VEColumn
                            { Table = Some aliasRef
                              Name = sqlFunSubEntity },
                        SQL.BOEq,
                        SQL.VEValue(SQL.VString entity.TypeName)
                    )
        ...
        let query =
            ...
            | Some role ->
                ...
                let usedDatabase =
                    singleKnownFlatEntity entity.Root entityRef usedEntityDelete usedFields

                let appliedDb =
                    try
                        applyPermissions layout role usedDatabase
                    with :? PermissionsApplyException as e when isUserException e ->
                        raise <| EntityOperationException(EOEAccessDenied(fullUserMessage e), e)
...
let private getSubEntity
    ...
    task {
        ...
        | _ ->
            ...
            let execute query args cancellationToken =
                connection.ExecuteRowValuesQuery query args cancellationToken
...
let private funRelatedId = OzmaQLName "related_id"
...
let private getRelatedRowIds
    ...
    task {
        ...
        let argumentValues =
            Map.unionUnique (Map.mapKeys PGlobal globalArgs) initialArgumentValues

        let opQuery =
            buildSelectOperation layout applyRole fieldRef.Entity
            <| fun aliasRef ->
                ...
                let usedFields =
                    Seq.singleton (
                        { Entity = Option.defaultValue fieldRef.Entity field.InheritedFrom
                          Name = fieldRef.Name },
                        usedFieldSelect
                    )
        ...
        let execute query args cancellationToken =
            connection.ExecuteQuery query args cancellationToken
            <| fun columns rows -> task { return! rows.Select(processOne).ToArrayAsync(cancellationToken) }
...
let getRelatedEntries
    ...
    task {
        ...
        let nodes =
            seq { 0 .. lastId - 1 } |> Seq.map (fun idx -> nodes.[idx]) |> Seq.toArray
