module OzmaDB.Operations.Update
...
let inline private updateDifference
    (db: DbContext)
    (updateFunc: 'k -> 'nobj -> 'eobj -> unit)
    (createFunc: 'k -> 'eobj)
    (deleteFunc: 'k -> 'eobj -> unit)
    (newObjects: Map<'k, 'nobj>)
    (existingObjects: Map<'k, 'eobj>)
    : Map<'k, 'eobj> =
...
let systemEntitiesMap: Lazy<HashMap<Type, ResolvedEntityRef>> =
    lazy
        (
         ...
         ctxType.GetProperties() |> Seq.mapMaybe (getDbSet funSchema db) |> HashMap.ofSeq)
    ...
    task {
        if not <| Set.isEmpty deferredSet then
            do!
                ...
                <| fun () ->
                    task {
                        let checkReference entityRef rowId (refFieldRef: ResolvedFieldRef) =
                            filterEntities refFieldRef.Entity
...
type SystemUpdater(db: SystemContext) =
    ...
    member inline this.UpdateDifference
        (updateFunc: 'k -> 'nobj -> 'eobj -> unit)
        (createFunc: 'k -> 'eobj)
        (newObjects: Map<'k, 'nobj>)
        (existingObjects: Map<'k, 'eobj>)
    ...
    member inline this.UpdateRelatedDifference
        (updateFunc: 'k -> 'nobj -> 'eobj -> unit)
        (newObjects: Map<'k, 'nobj>)
        (existingObjects: Map<'k, 'eobj>)
        : Map<'k, 'eobj> =
        let createFunc name =
            raisef SystemUpdaterException "Object %O doesn't exist" name

        let deleteFunc name obj =
            raisef SystemUpdaterException "Refusing to delete object %O" name
...
let deleteDeferredFromUpdate
    (layout: Layout)
    (connection: DatabaseTransaction)
    (result: UpdateResult)
    (cancellationToken: CancellationToken)
    =
    cascadeDeleteDeferred
        (fun entityRef -> entityRef.Schema = funSchema)
        layout
        connection
        result.DeferredDeletes
        cancellationToken

let private schemaEntityRef: ResolvedEntityRef =
    { Schema = funSchema
...
let genericSystemUpdate<'updater when 'updater :> SystemUpdater>
    (db: SystemContext)
    (cancellationToken: CancellationToken)
    (getUpdater: unit -> Task<'updater>)
    : Task<UpdateResult> =
    task {
...
let private makeEntity schemaName (entity: Entity) =
    ({ Schema = schemaName
       Name = OzmaQLName entity.Name },
     entity)

let private makeSchema (schema: Schema) =
    schema.Entities |> Seq.ofObj |> Seq.map (makeEntity (OzmaQLName schema.Name))
