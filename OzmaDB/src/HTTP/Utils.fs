...
type RequestErrorInfo =
    ...
    member this.Error = RequestErrorInfo.LookupKey this |> Option.get
...
let requestError<'Error when 'Error :> IErrorDetails> (e: 'Error) =
    setHttpHeader "X-OzmaDB-Error" e.Error
    >=> setStatusCode e.HTTPResponseCode
    >=> json e
...
let notFoundHandler: HttpFunc -> HttpContext -> HttpFuncResult =
    requestError RINoEndpoint
...
let private processArgs
    (rawArgs: KeyValuePair<string, StringValues> seq)
    (f: Map<string, JToken> -> HttpHandler)
    : HttpHandler =
...
let jobJson<'Response> (r: 'Response) : HttpJobResponse =
    let header =
        { ResponseCode = 200
          Headers = dict <| seq { ("Content-Type", "application/json") } }

    (header, jsonJobDataWriter r)

let inline commitAndReturn
    (api: IOzmaDBAPI)
...
let inline commitAndOk (api: IOzmaDBAPI) : Task<HttpJobResponse> =
    commitAndReturn api (fun () -> jobJson Map.empty)

let inline jobReplyWithCommit<'Response, 'Error when 'Error :> IErrorDetails>
    (api: IOzmaDBAPI)
    (ret: Result<'Response, 'Error>)
    =
    match ret with
...
let inline private limitRequestTime
    (instance: IInstance)
    ([<InlineIfLambda>] f: CancellationToken -> Task<HttpJobResponse>)
    ...
    task {
        ...
        | Some maxTime ->
            ...
            ignore
            <| cancellationToken.UnsafeRegister((fun source -> (source :?> CancellationTokenSource).Cancel()), newToken)

            newToken.CancelAfter(int maxTime.TotalMilliseconds)

            try
                return! f newToken.Token
            with Exn.Innermost(:? OperationCanceledException as e) when
                newToken.IsCancellationRequested
...
type HttpJobUtils
    ...
    let withUser (f: UserTokenInfo -> HttpHandler) (next: HttpFunc) (ctx: HttpContext) =
        ...
        if isNull client then
            ...
            let email = if isNull emailClaim then None else Some emailClaim.Value
    ...
    let lookupInstance (ctx: HttpContext) =
        let instanceName =
            match ctx.Request.Headers.TryGetValue("X-OzmaDB-Instance") with
    ...
    let withInstanceContext (f: InstanceContext -> HttpHandler) (next: HttpFunc) (ctx: HttpContext) =
        task {
            ...
            | None -> return! requestError RINoInstance next ctx
            | Some instance ->
                use _ = instance

                match instance.Region with
                | Some instanceRegion -> ctx.SetHttpHeader("X-OzmaDB-Region", instanceRegion)
                | _ -> ()
    ...
    let sendHttpJobResponse
        ...
        task {
            ...
            let setSize =
                function
                | None -> ()
                | Some size -> ctx.Response.Headers.ContentLength <- Nullable(size)

            do! streamJobData rmsManager resultWriter ctx.Response.Body setSize ctx.RequestAborted
            return Some ctx
        }

    let withRemoteJob (f: RemoteJobSettings -> HttpHandler) (next: HttpFunc) (ctx: HttpContext) =
    ...
    let startRemoteJob (f: RemoteJobSettings -> Task<RemoteJobRef>) =
    ...
    let wrapAsJob (f: CancellationToken -> Task<HttpJobResponse>) (next: HttpFunc) (ctx: HttpContext) : HttpFuncResult =
        task {
            let longRunning =
                match ctx.Request.Headers.TryGetValue("X-OzmaDB-LongRunning") with
                | (true, values) ->
            ...
            | None -> return! requestError (RIRequest "Invalid X-OzmaDB-LongRunning value") next ctx
            | Some LRNo ->
                try
            ...
            | Some LRYes ->
                let job remoteSettings =
                    task {
                        use longJob =
                            new LongRunningJob<HttpJobResponseHeader>(
                                rmsManager,
                                jobResponseLogger,
            ...
            | Some LRHybrid ->
                ...
                | Some res -> return! sendHttpJobResponse res next ctx
                | None -> return! startRemoteJob (longJob.ConvertToRemote) next ctx
        }
    ...
    let runWithApi
        ...
        task {
            logger.LogDebug(
                "Creating context for instance {instance}, user {user} (is root: {isRoot})",
                ictx.Instance.Name,
                ictx.UserName,
                ictx.IsRoot
            )

            let maxConcurrentRetries = 15
            let baseDelayMs = 100
            let maxDelayMs = 5000
            let mutable attempt = 0
                ...
                | :? ConcurrentUpdateException as e ->
                    if attempt >= maxConcurrentRetries then
                        ...
                        let exponentialDelay =
                            if attempt >= 30 then maxDelayMs
                            else baseDelayMs * (1 <<< min attempt 20)
                        let delay = min maxDelayMs (exponentialDelay + jitter)
    ...
    let checkRateLimit
        (prefix: string)
        (getLimits: IInstance -> RateLimit seq)
        (inst: IInstance)
        (next: HttpFunc)
        (ctx: HttpContext)
        =
    ...
    member this.WithHiddenContext(f: IOzmaDBAPI -> Task<HttpJobResponse>) : HttpHandler =
        withInstanceContext (fun inst -> runJobWithApi false inst f)

    member this.PerformReadJob(f: IOzmaDBAPI -> Task<HttpJobResponse>) =
        withRateLimitCheck "rate_limit:read:" (fun inst -> inst.ReadRateLimitsPerUser) f

    member this.PerformWriteJob(f: IOzmaDBAPI -> Task<HttpJobResponse>) =
        withRateLimitCheck "rate_limit:write:" (fun inst -> inst.WriteRateLimitsPerUser) f
