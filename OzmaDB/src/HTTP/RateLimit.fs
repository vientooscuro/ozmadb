...
type ContextRateLimitProcessor
    (options: IOptions<ClientRateLimitOptions>, strategy: IProcessingStrategy, contextAccessor: IHttpContextAccessor) =
...
type private ContextRateLimitMiddleware
    (
        next: RequestDelegate,
        quotaExceeded: string -> HttpHandler,
    ...
    override this.LogBlockedRequest(httpContext, identity, counter, rule) =
        logger.LogInformation(
            "Request for {client_id} has been blocked, quota {limit}/{period} exceeded by {excess}",
            identity.ClientId,
            rule.Limit,
            rule.Period,
...
type RateLimiter
    ...
    member this.CheckRateLimit (quotaExceeded: string -> HttpHandler) (next: HttpFunc) (ctx: HttpContext) =
        task {
            ...
            let middleware =
                ContextRateLimitMiddleware(delegateNext, quotaExceeded, options.Value, processor, config, logger)
