...
let private findTriggersTime
    (entity: ResolvedEntityRef)
    (time: TriggerTime)
    (triggers: MergedTriggers)
    : MergedTriggersTime option =
    ...
    |> Option.map (fun entity ->
        match time with
        | TTBefore -> entity.Before
        | TTAfter -> entity.After)

let findMergedTriggersInsert
    (entity: ResolvedEntityRef)
    (time: TriggerTime)
    (triggers: MergedTriggers)
    : MergedTrigger seq =
...
let emptyMergedTriggers: MergedTriggers = { Schemas = Map.empty }

let private mergeSortedTriggers (a: MergedTrigger[]) (b: MergedTrigger[]) : MergedTrigger[] =
    Seq.mergeSortedBy (fun trig -> (trig.Priority, trig.Inherited, trig.Schema, trig.Name)) a b
    |> Array.ofSeq
...
let private makeOneMergedTriggerEntity
    (schemaName: SchemaName)
    (name: TriggerName)
    (trigger: ResolvedTrigger)
    : MergedTriggersEntity option =
    if
        not (
            trigger.OnInsert
...
let private markEntityInherited
    (originalRef: ResolvedEntityRef)
...
type private TriggersMerger(layout: Layout) =
    let emitMergedTriggersEntity
        ...
        (triggerEntityRef: ResolvedEntityRef)
        (triggerEntity: ResolvedEntity)
        (trigger: ResolvedTrigger)
        =
        seq {
