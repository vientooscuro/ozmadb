...
type UserViewsAPI(api: IOzmaDBAPI) =
    ...
    let resolveSource
        (source: UserViewSource)
        (flags: UserViewFlags)
        : Task<Result<PrefetchedUserView, UserViewErrorInfo>> =
        task {
            try
                match source with
                | UVAnonymous query ->
                ...
                | Ok uv ->
                    try
                        ...
                        | RTRole role ->
                            ...
                            <| applyPermissions
                                ctx.Layout
                                (Option.defaultValue emptyResolvedRole role.Role)
                                uv.UserView.Compiled.UsedDatabase
                    ...
                    with :? PermissionsApplyException as ex when ex.IsUserException ->
            ...
            task {
                if not (canExplain rctx.User.Saved.Type) then
                    ...
                    | Ok uv ->
                        ...
                        | :? ArgumentCheckException as e -> return Error <| UVEExecution(UVEArgument e.Details)
                            ...
                            task {
                                ...
                                return
                                    { ArgumentAttributes =
                                        Map.unionWith
                                            Map.unionUnique
                                            uv.ConstAttributes.ArgumentAttributes
                                            res.ArgumentAttributes
                                      Attributes = Map.unionUnique uv.ConstAttributes.Attributes res.Attributes
                                      ColumnAttributes =
                                        Array.map2
                                            Map.unionUnique
                                            uv.ConstAttributes.ColumnAttributes
                                            res.ColumnAttributes
                                      Rows = rows }
                            }
