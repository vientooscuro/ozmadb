...
type DomainsAPI(api: IOzmaDBAPI) =
    ...
    member this.GetDomainValues(req: GetDomainValuesRequest) : Task<Result<DomainValuesResponse, DomainErrorInfo>> =
        wrapAPIResult rctx "getDomainValues" req
        <| fun () ->
            task {
                ...
                | Some domain ->
                    try
                        ...
                        let (expr, argValues) =
                            ...
                            | (Some(RRKPrimary id), Some rowSpecific) ->
                                (rowSpecific, Map.add (PLocal funId) (FInt id) argValues)
                        ...
                        let comment =
                            domainComments req.Field rctx.User.Effective.Type req.Id rctx.GlobalArguments chunk
    ...
    member this.GetDomainExplain(req: GetDomainExplainRequest) : Task<Result<ExplainedQuery, DomainErrorInfo>> =
        wrapAPIResult rctx "getDomainExplain" req
        <| fun () ->
            task {
