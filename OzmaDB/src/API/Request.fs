...
let private fetchUser
    (system: SystemContext)
    (userName: UserName)
    (allowDisabled: bool)
    (cancellationToken: CancellationToken)
    : Task<FetchedUser option> =
    task {
        ...
        if isNull user then
            ...
            let role =
                if isNull user.Role then
                    ...
                    Some
                        { Schema = OzmaQLName user.Role.Schema.Name
                          Name = OzmaQLName user.Role.Name }
...
type RequestContext private (ctx: IContext, initialUserInfo: RequestUserInfo, opts: RequestArguments, logger: ILogger) =
    ...
    member this.WriteEvent(addDetails: EventEntry -> unit) = ctx.WriteEvent(makeEvent addDetails)
    ...
    member this.RunWithSource (newSource: EventSource) (func: unit -> Task<'a>) : Task<'a> =
        task {
            ...
            try
                try
                    return! func ()
                with :? RequestStackOverflowException as e ->
