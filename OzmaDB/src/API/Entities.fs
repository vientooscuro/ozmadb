...
type MassInsertRow = { Args: LocalArgumentsMap; Id: int }
...
let convertRawRowKey (entity: ResolvedEntity) =
    function
    | RRKPrimary id -> RKPrimary id
    | RRKAlt(name, rawArgs) -> RKAlt(name, convertEntityArguments entity rawArgs)

let private usersEntityRef =
    { Schema = OzmaQLName "public"
      Name = OzmaQLName "users" }
...
type EntitiesAPI(api: IOzmaDBAPI) =
    ...
    let runArgsTrigger
        ...
        : Task<Result<LocalArgumentsMap, BeforeTriggerError<unit>>> =
        let ref =
    ...
    let runAfterTrigger
        ...
        (entityRef: ResolvedEntityRef)
        (trigger: MergedTrigger)
        : Task<Result<unit, EntityErrorInfo>> =
        let ref =
            { Schema = trigger.Schema
              Entity = Option.defaultValue entityRef trigger.Inherited
              Name = trigger.Name }
    ...
    let applyInsertTriggerBefore
        ...
        : Task<Result<LocalArgumentsMap, BeforeTriggerError<unit>>> =
        runArgsTrigger
            (fun script -> script.RunInsertTriggerBefore entityRef args ctx.CancellationToken)
            entityRef
            entity
            args
            trigger

    let applyUpdateTriggerBefore
        (entityRef: ResolvedEntityRef)
        (entity: ResolvedEntity)
        (key: RowKey, args: LocalArgumentsMap)
        (trigger: MergedTrigger)
        : Task<Result<RowKey * LocalArgumentsMap, BeforeTriggerError<RowId>>> =
        task {
            let! id =
                resolveKey
                    query
            ...
            match!
                runArgsTrigger
                    (fun script -> script.RunUpdateTriggerBefore entityRef id args ctx.CancellationToken)
                    entityRef
                    entity
                    args
                    trigger
    ...
    let applyDeleteTriggerBefore
        (entityRef: ResolvedEntityRef)
        (key: RowKey)
        (trigger: MergedTrigger)
        : Task<Result<RowKey, BeforeTriggerError<RowId>>> =
        let ref =
            { Schema = trigger.Schema
              Entity = Option.defaultValue entityRef trigger.Inherited
              Name = trigger.Name }
        ...
        <| fun () ->
            task {
                ...
                try
                    let! maybeContinue =
                        preparedTrigger.Script.RunDeleteTriggerBefore entityRef id ctx.CancellationToken
                ...
                with :? TriggerRunException as ex when ex.IsUserException ->
                    ...
                    return
                        Error
                        <| BEError(EETrigger(trigger.Schema, trigger.Name, EEException(str, ex.UserData)))
            }

    let applyInsertTriggerAfter
        (entityRef: ResolvedEntityRef)
        (id: int)
        (args: LocalArgumentsMap)
        ()
        ...
        : Task<Result<unit, EntityErrorInfo>> =
        runAfterTrigger
            (fun script -> script.RunInsertTriggerAfter entityRef id args ctx.CancellationToken)
            entityRef
            trigger

    let applyUpdateTriggerAfter
        (entityRef: ResolvedEntityRef)
        (id: int)
        (args: LocalArgumentsMap)
        ()
        ...
        : Task<Result<unit, EntityErrorInfo>> =
        runAfterTrigger
            (fun script -> script.RunUpdateTriggerAfter entityRef id args ctx.CancellationToken)
            entityRef
            trigger

    let applyDeleteTriggerAfter
        (entityRef: ResolvedEntityRef)
        ()
        (trigger: MergedTrigger)
        : Task<Result<unit, EntityErrorInfo>> =
    ...
    member this.GetEntityInfo(req: GetEntityInfoRequest) : Task<Result<SerializedEntity, EntityErrorInfo>> =
        wrapAPIResult rctx "getEntityInfo" req
        <| fun () ->
            task {
                ...
                | Some entity ->
                    try
                        let res =
                            getEntityInfo ctx.Layout ctx.Triggers (getReadRole rctx.User.Effective.Type) req.Entity

                        return Ok res
                    with :? EntityOperationException as ex when ex.IsUserException ->
    ...
    member this.InsertEntries(req: InsertEntriesRequest) : Task<Result<InsertEntriesResponse, TransactionErrorInfo>> =
        wrapAPIError rctx "insertEntries" req
        <| fun () ->
            task {
                match ctx.Layout.FindEntity req.Entity with
                | None ->
                    ...
                    return
                        Error
                            { Inner = EEOperation(EOERequest err)
                              Operation = 0 }
                ...
                | Some entity ->
                    try
                        ...
                        let runSingleId
                            (i, (rawArgs: RawArguments, rowArgs: LocalArgumentsMap))
                            : Task<Result<InsertEntryResponse, TransactionErrorInfo>> =
                            task {
                                match!
                                    Seq.foldResultTask
                                        (applyInsertTriggerBefore req.Entity entity)
                                        rowArgs
                                        beforeTriggers
                                with
                                ...
                                | Ok args ->
                                    ...
                                    match!
                                        Seq.foldResultTask
                                            (applyInsertTriggerAfter req.Entity newId args)
                                            ()
                                            afterTriggers
                                    with
                        ...
                        let convertOne (i, rowArgs) =
                            ...
                            with :? ArgumentCheckException as e when e.IsUserException ->
                                logger.LogError(e, "Invalid arguments for entity insert")

                                Error
                                    { Inner = EEOperation(EOEExecution(UVEArgument e.Details))
                                      Operation = i }
                            ...
                            let! ret =
                                task {
                                    if not (Seq.isEmpty beforeTriggers && Seq.isEmpty afterTriggers) then
                                        let! ret =
                                            Seq.traverseResultTask
                                                runSingleId
                                                (Seq.indexed (Seq.zip req.Entries rowsArgs))
                ...
                | Some entity ->
                    try
                        ...
                        if Map.isEmpty args then
                            ...
                            match!
                                Seq.foldResultTask
                                    (applyUpdateTriggerBefore req.Entity entity)
                                    (key, args)
                                    beforeTriggers
                            with
                            ...
                            | Ok(key, args) ->
                                ...
                                let afterTriggers =
                                    findMergedTriggersUpdate req.Entity TTAfter (Map.keys args) ctx.Triggers
    ...
    member this.DeleteEntry(req: DeleteEntryRequest) : Task<Result<unit, EntityErrorInfo>> =
        wrapAPIError rctx "deleteEntry" req
        <| fun () ->
            task {
                match ctx.Layout.FindEntity req.Entity with
                | None ->
    ...
    member this.GetRelatedEntries(req: GetRelatedEntriesRequest) : Task<Result<ReferencesTree, EntityErrorInfo>> =
        wrapAPIResult rctx "getRelatedEntries" req
        <| fun () ->
            task {
                match ctx.Layout.FindEntity(req.Entity) with
                | None ->
        ...
        task {
            ...
            | Ok tree ->
                ...
                try
                    ...
                    return Ok tree
                with :? EarlyStopException<EntityErrorInfo> as e ->
            ...
            task {
                try
                    ...
                    for KeyValue(entityRef, usedEntity) in cmd.UsedDatabase do
                        ...
                        if
                            entity.TriggersMigration
                            && anyChildren (fun ent -> ent.Insert || ent.Update || ent.Delete)
                        then
                            ctx.ScheduleMigration()
                    ...
                    return Ok()
                with :? CommandException as e when e.IsUserException ->
    ...
    member this.RunTransaction(req: TransactionRequest) : Task<Result<TransactionResponse, TransactionErrorInfo>> =
        task {
            logAPIRequest rctx "runTransaction" req
                ...
                task {
                    ...
                    | Error e ->
                        return
                            Error
                                { e with
                                    Operation = pending.StartIndex + e.Operation }
            ...
            let handleOne (i, op) =
                let inline checkAndRun
                    (resultFn: 'a -> TransactionOpResponse)
                    (opFn: unit -> Task<Result<'a, EntityErrorInfo>>)
                    =
                    task {
                ...
                task {
                            ...
                            pendingInsert <-
                                Some
                                    { Entity = insert.Entity
                                      Entries = List(seq { insert.Fields })
                                      StartIndex = i }
                    ...
                    | TDeleteEntity delete ->
                        return! checkAndRunConst TRDeleteEntity (fun () -> this.DeleteEntry delete)
                    | TRecursiveDeleteEntity delete ->
                        return! checkAndRun TRRecursiveDeleteEntity (fun () -> this.RecursiveDeleteEntry delete)
    ...
    member this.DeferConstraints(func: unit -> Task<'a>) : Task<Result<'a, EntityErrorInfo>> =
        task {
            logAPIRequest rctx "deferConstraints" emptyRequest

            try
                let! ret = ctx.Transaction.DeferConstraints ctx.CancellationToken func
                return Ok ret
            with :? DeferredConstraintsException as e when e.IsUserException ->
