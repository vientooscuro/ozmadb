...
type ForeignKeyMeta =
    ...
    member this.ToSQLString() =
        let myCols =
            this.Columns
            |> Seq.map (fun (name, refName) -> name.ToSQLString())
            |> String.concat ", "

        let refCols =
            this.Columns
            |> Seq.map (fun (name, refName) -> refName.ToSQLString())
            |> String.concat ", "

        sprintf
            "(%s) REFERENCES %s (%s) ON DELETE %O ON UPDATE %O %O"
            myCols
            (this.ToTable.ToSQLString())
            refCols
            this.OnDelete
            this.OnUpdate
            this.Defer
...
type ConstraintMeta =
    ...
    member this.ToSQLString() =
        ...
        | CMForeignKey opts -> sprintf "FOREIGN KEY %O" opts
...
let private functionArgumentToString (signature: FunctionArgumentSignature) (arg: FunctionArgument) =
    ...
    String.concatWithWhitespaces
        [ signature.Mode.ToSQLString()
          nameStr
          signature.TypeName.ToSQLString()
          defaultStr ]
...
let unionDatabaseMeta (a: DatabaseMeta) (b: DatabaseMeta) =
    { Schemas =
        Map.unionWith
            (fun (keys1, meta1) (keys2, meta2) -> (Set.union keys1 keys2, unionSchemaMeta meta1 meta2))
            a.Schemas
            b.Schemas
...
let columnType (newType: DBValueType) : AlterColumnType = { NewType = newType; Using = None }
...
type TableOperation =
    ...
    member this.ToSQLString() =
        ...
        | TOAlterColumnNull(col, isNullable) ->
            sprintf "ALTER COLUMN %s %s NOT NULL" (col.ToSQLString()) (if isNullable then "DROP" else "SET")
        | TOAlterColumnDefault(col, None) -> sprintf "ALTER COLUMN %s DROP DEFAULT" (col.ToSQLString())
        | TOAlterColumnDefault(col, Some def) ->
            sprintf "ALTER COLUMN %s SET DEFAULT %s" (col.ToSQLString()) (def.ToSQLString())
...
[<StructuralEquality; NoComparison>]
type TemporaryTableOpts = { OnCommit: CommitTableAction option }
...
type CreateTableOperation =
    ...
    member this.ToSQLString() =
        ...
        String.concatWithWhitespaces
            [ "CREATE"
              tmpStr
              sprintf "TABLE %s ()" (this.Table.ToSQLString())
              onCommitStr ]
...
type SchemaOperation =
    ...
    member this.ToSQLString() =
        ...
        | SORenameSchema(schema, toName) ->
            sprintf "ALTER SCHEMA %s RENAME TO %s" (schema.ToSQLString()) (toName.ToSQLString())
        | SODropSchema schema -> sprintf "DROP SCHEMA %s" (schema.ToSQLString())
        | SOCreateTable opts -> opts.ToSQLString()
        | SORenameTable(table, toName) ->
            sprintf "ALTER TABLE %s RENAME TO %s" (table.ToSQLString()) (toName.ToSQLString())
        | SOAlterTable(table, ops) ->
            sprintf "ALTER TABLE %s %s" (table.ToSQLString()) (ops |> Seq.map toSQLString |> String.concat ", ")
        | SORenameTableColumn(table, col, toCol) ->
            sprintf
        ...
        | SORenameConstraint(table, name, toName) ->
            sprintf "ALTER TABLE %O RENAME CONSTRAINT %s TO %s" table (toSQLString name) (toSQLString toName)
        | SOAlterConstraint(table, name, alter) ->
            sprintf "ALTER TABLE %O ALTER CONSTRAINT %s %O" table (toSQLString name) alter
        ...
        | SOCreateIndex(index, table, pars) ->
            ...
            let suffixStr =
                sprintf
                    "INDEX %s ON %s USING %s (%s)"
                    (index.Name.ToSQLString())
                    ({ index with Name = table }.ToSQLString())
                    (pars.AccessMethod.ToSQLString())
                    columnsStr
        ...
        | SORenameIndex(index, toName) ->
            sprintf "ALTER INDEX %s RENAME TO %s" (index.ToSQLString()) (toName.ToSQLString())
        ...
        | SOCreateOrReplaceFunction(func, args, def) ->
            ...
            let argsStr =
                Seq.map2 functionArgumentToString args def.Arguments |> String.concat ", "

            sprintf
                "CREATE OR REPLACE FUNCTION %s (%s) RETURNS %s LANGUAGE %s %s AS %s"
                (func.ToSQLString())
                argsStr
        ...
        | SOCreateTrigger(table, name, def) ->
            ...
            let triggerStr =
                sprintf "TRIGGER %s %s %s ON %O" (toSQLString name) (def.Order.ToSQLString()) eventsStr table
            ...
            let tailStr =
                sprintf "EXECUTE FUNCTION %s (%s)" (def.FunctionName.ToSQLString()) argsStr
        ...
        | SODropTrigger(table, name) -> sprintf "DROP TRIGGER %s ON %O" (toSQLString name) table
