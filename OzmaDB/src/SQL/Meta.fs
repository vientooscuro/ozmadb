...
open OzmaDB.SQL.Array.Lex
open OzmaDB.SQL.Array.Parse
...
let private parseToSimpleType: SimpleType -> (string -> Value) =
    function
    | STString -> VString
...
let private makeColumnMeta (attr: Attribute) : ColumnMeta =
    try
        ...
        let genExpr =
            attr.AttrDef
            |> Option.ofObj
            |> Option.map (fun def -> parseLocalExpr def.Source)
    ...
    with e ->
        raisefWithInner SQLMetaException e "In column %s" attr.AttName
...
let private makeTriggerMeta (columnIds: TableColumnIds) (trigger: Trigger) : TriggerName * TriggerDefinition =
    try
        ...
        let condition =
            ...
            | Regex @"WHEN \((.*)\) EXECUTE (?:PROCEDURE|FUNCTION)" [ cond ] ->
                parseLocalExpr cond |> String.comparable |> Some
            | _ -> None
    ...
    with e ->
        raisefWithInner SQLMetaException e "In trigger %s" trigger.TgName
...
let private makeUnconstrainedTableMeta (cl: Class) : TableName * HalfTableMeta =
    try
        let columnIds =
            cl.Attributes
            |> Seq.ofObj
            |> Seq.map (fun attr -> (attr.AttNum, SQLName attr.AttName))
            |> Map.ofSeqUnique

        let columns =
            cl.Attributes
            |> Seq.ofObj
            |> Seq.map (fun col -> (SQLName col.AttName, (Set.empty, makeColumnMeta col)))
            |> Map.ofSeqUnique
        ...
        (tableName, meta)
    with e ->
        raisefWithInner SQLMetaException e "In table %s" cl.RelName
...
let private makeUnconstrainedSchemaMeta (ns: Namespace) : SchemaName * PgSchemaMeta =
    try
        let tablesMeta =
            ns.Classes
            |> Seq.filter (fun cl -> cl.RelKind = 'r')
            |> Seq.map makeUnconstrainedTableMeta
            |> Map.ofSeqUnique

        let sequences =
            ns.Classes
            |> Seq.filter (fun cl -> cl.RelKind = 'S')
            |> Seq.map makeSequenceMeta
            |> Map.ofSeqUnique
    ...
    with e ->
        raisefWithInner SQLMetaException e "In schema %s" ns.NspName
...
type private Phase2Resolver(schemaIds: PgSchemas) =
    ...
    let makeConstraintMeta
        (tableName: TableName)
        (columnIds: TableColumnIds)
        (constr: Constraint)
        : (ConstraintName * ConstraintMeta) option =
        let makeLocalColumn (num: ColumnNum) = Map.find num columnIds

        let ret =
            match constr.ConType with
            | 'c' -> parseLocalExpr constr.Source |> String.comparable |> CMCheck |> Some
            | 'f' -> Some <| CMForeignKey(makeForeignKeyMeta columnIds constr)
            | 'p' ->
                Some
                <| CMPrimaryKey(Array.map makeLocalColumn constr.ConKey, makeDeferrableConstraint constr)
            | 'u' ->
                Some
                <| CMUnique(Array.map makeLocalColumn constr.ConKey, makeDeferrableConstraint constr)
            | _ -> None
    ...
    let makeIndexMeta (columnIds: TableColumnIds) (index: Index) : (IndexName * IndexMeta) =
        ...
        let makeColumn (i: int) =
            ...
            let order =
                if not canOrder then None
                else if options.HasFlag IndexOptions.Desc then Some Desc
                else Some Asc

            let nullsOrder =
                if not canOrder then
                    None
                else if options.HasFlag IndexOptions.NullsFirst then
                    Some NullsFirst
                else
                    Some NullsLast
        ...
        let columns =
            seq { for i in 0 .. int index.IndNKeyAtts - 1 -> makeColumn i } |> Seq.toArray

        let includedColumns =
            seq { for i in int index.IndNKeyAtts .. index.IndKey.Length - 1 -> makeKey i }
            |> Seq.toArray
...
let createPgCatalogContext (transaction: DatabaseTransaction) =
    ...
    ignore
    <| builder.UseNpgsql(transaction.Transaction.Connection, (fun opts -> ignore <| opts.UseNodaTime()))
...
let buildDatabaseMeta (transaction: DatabaseTransaction) (cancellationToken: CancellationToken) : Task<DatabaseMeta> =
    task {
        ...
        let unconstrainedSchemas =
            namespaces |> Seq.map makeUnconstrainedSchemaMeta |> Map.ofSeqUnique

        let phase2 = Phase2Resolver(unconstrainedSchemas)

        let schemas =
            unconstrainedSchemas
            |> Map.mapWithKeys (fun name meta -> (name, (Set.empty, phase2.FinishSchemaMeta name meta)))
