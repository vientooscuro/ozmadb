...
type private NaiveRenamer(renamesMap: RenamesMap) =
    let rec renameSelectTreeExpr: SelectTreeExpr -> SelectTreeExpr =
        function
    ...
    and renameSelectedColumn: SelectedColumn -> SelectedColumn =
        function
        | SCAll _ -> failwith "Unexpected SELECT *"
        | SCExpr(name, expr) -> SCExpr(name, renameValueExpr expr)

    and renameValueExpr =
        let mapColumn: ColumnRef -> ColumnRef =
            function
            | { Table = Some { Schema = None; Name = tableName }
                Name = colName } as ref ->
                match Map.tryFind tableName renamesMap with
                | None -> ref
                | Some newName ->
                    { Table = Some { Schema = None; Name = newName }
                      Name = colName }
            | ref -> ref
    ...
    and renameTableExpr: TableExpr -> TableExpr =
        function
    ...
    and renameFromExpr: FromExpr -> FromExpr =
        function
