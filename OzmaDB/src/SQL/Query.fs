...
let private convertDecimal: obj -> decimal option =
    function
...
let private convertValueOrThrow (valType: SimpleValueType) (rawValue: obj) =
    ...
    | (VTArray scalarType, (:? Array as rootVals)) ->
        let rec convertArray (convFunc: obj -> 'a option) (vals: Array) : ValueArray<'a> =
            let convertOne: obj -> ArrayValue<'a> =
                function
                | :? DBNull -> AVNull
        ...
        | STJson ->
            VJsonArray(convertArray (tryCast<string> >> Option.bind tryJson >> Option.map ComparableJToken) rootVals)
...
let private npgsqlValue: Value -> NpgsqlDbType option * obj =
    function
...
type QueryConnection(loggerFactory: ILoggerFactory, connection: NpgsqlConnection) =
    ...
    member this.ExecuteRowValuesQuery
        (queryStr: string)
        (pars: ExprParameters)
        (cancellationToken: CancellationToken)
        : Task<(SQLName * SimpleValueType * Value)[] option> =
                ...
                if not hasRow0 then
                    ...
                    let getRow () =
                        try
                            ...
                            res
                        with e ->
                            raisefWithInner QueryException e "While reading row %i" rowI
    ...
    member this.ExecuteColumnValuesQuery
        (queryStr: string)
        (pars: ExprParameters)
        (cancellationToken: CancellationToken)
...
let runExplainQuery<'a when 'a :> ISQLString>
