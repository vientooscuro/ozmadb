...
let rec mapArrayValue (func: 'a -> 'b) : ArrayValue<'a> -> ArrayValue<'b> =
    function
    | AVValue a -> AVValue(func a)
    | AVArray vals -> AVArray(mapValueArray func vals)
    | AVNull -> AVNull

and mapValueArray (func: 'a -> 'b) (vals: ValueArray<'a>) : ValueArray<'b> = Array.map (mapArrayValue func) vals
...
// Used when interpreting query results and for compiling OzmaQL.
...
let mapValueType (func: 'a -> 'b) : ValueType<'a> -> ValueType<'b> =
    function
...
let findSimpleValueType: DBValueType -> SimpleValueType option =
    function
...
type ValueExpr =
    ...
    member this.ToSQLString() =
        ...
        | VECase(es, els) ->
            let esStr =
                es
                |> Seq.map (fun (cond, e) -> sprintf "WHEN %s THEN %s" (cond.ToSQLString()) (e.ToSQLString()))
                |> String.concat " "
...
and [<NoEquality; NoComparison>] CommonTableExprs =
    ...
    member this.ToSQLString() =
        ...
        let exprs = this.Exprs |> Seq.map oneExpr |> String.concat ", "
    ...
    member this.ToSQLString() =
        ...
        let insertStr =
            sprintf
                ...
                (this.Columns
                 |> Seq.map (fun (extra, x) -> x.ToSQLString())
                 |> String.concat ", ")
...
let rec private normalizeArrayValue (constr: 'a -> Value) : ArrayValue<'a> -> ValueExpr =
    function
...
let updateColumnName (name: ColumnName) : UpdateColumnName = { Name = name; Extra = null }

let commonTableExprs (exprs: (TableName * CommonTableExpr)[]) : CommonTableExprs = { Recursive = false; Exprs = exprs }
