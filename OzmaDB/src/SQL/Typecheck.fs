...
let sqlSimpleTypes =
    unionCases typeof<SimpleType>
    |> Seq.map (fun case -> FSharpValue.MakeUnion(case.Info, [||]) :?> SimpleType)
    |> Set.ofSeq
...
let sqlTypeCategories: TypeCategoriesMap =
    Map.ofList
        [ ('N',
           Map.ofList
               [ (VTScalar STDecimal, true)
                 (VTScalar STInt, false)
                 (VTScalar STRegclass, true)
                 (VTScalar STBigInt, false) ])
          ('D',
           Map.ofList
               [ (VTScalar STDateTime, true)
                 (VTScalar STLocalDateTime, false)
                 (VTScalar STDate, false) ])
...
let private checkAllCategories () =
    let sqlAllTypes =
        Seq.append (Seq.map VTScalar sqlSimpleTypes) (Seq.map VTArray sqlSimpleTypes)
        |> Set.ofSeq

    let foundTypes =
        sqlTypeCategories |> Map.values |> Seq.collect Map.keys |> Set.ofSeq
...
let sqlReverseTypeCategories: ReverseTypeCategoriesMap =
    sqlTypeCategories
    |> Map.toSeq
    |> Seq.collect (fun (cat, typs) ->
        typs
        |> Map.toSeq
        |> Seq.map (fun (typ, preferred) ->
            (typ,
...
let sqlPreferredTypes: Set<SimpleValueType> =
    ...
    |> Seq.collect (
        Map.toSeq
        >> Seq.mapMaybe (fun (typ, preferred) -> if preferred then Some typ else None)
    )
    |> Set.ofSeq
...
let private toCharSignatures: FunctionSignaturesMap =
    [ STDateTime; STLocalDateTime; STInterval; STInt; STDecimal ]
...
let sqlKnownFunctions: Map<FunctionName, FunctionSignaturesMap> =
    Map.ofList
        [ // Common
          ...
          (SQLName "substr",
           funScalarsToSignatures [ ([ STString; STInt ], STString); ([ STString; STInt; STInt ], STString) ])
          ...
          // Dates
          (SQLName "age",
           funScalarsToSignatures [ ([ STDateTime; STDateTime ], STInterval); ([ STDateTime ], STInterval) ])
          (SQLName "date_part",
           funScalarsToSignatures [ ([ STString; STDateTime ], STDecimal); ([ STString; STInterval ], STDecimal) ])
          (SQLName "date_trunc",
           funScalarsToSignatures
               [ ([ STString; STLocalDateTime ], STLocalDateTime)
...
let private binScalarsToSignatures (signs: ((SimpleType * SimpleType) * SimpleType) seq) : BinaryOperatorSignaturesMap =
    signs
    |> Seq.map (fun ((a, b), ret) -> ((HTScalar(Some a), HTScalar(Some b)), HTScalar(Some ret)))
    |> Map.ofSeq

let private symmetricOpSignatures
    (signatures: ((SimpleType * SimpleType) * SimpleType) seq)
    : BinaryOperatorSignaturesMap =
    Seq.append
        (signatures |> Seq.map (fun ((a, b), ret) -> ((a, b), ret)))
        (signatures |> Seq.map (fun ((a, b), ret) -> ((b, a), ret)))
...
let private likeOverloads =
    [ ((STString, STString), STBool) ] |> binScalarsToSignatures
...
let private jsonTextArrowOverloads =
    [ ((STJson, STString), STString); ((STJson, STInt), STString) ]
    |> binScalarsToSignatures

let binaryOperatorSignature =
    function
    | BOLess
    | BOLessEq
    | BOGreater
...
let private expandHoledSignatures
    (signature: HoledValueType[])
    (ret: HoledValueType)
    (typs: SimpleValueType option[])
    : (SimpleValueType[] * SimpleValueType) seq =
    ...
    | None -> Seq.empty
    | Some pls when Set.isEmpty pls -> Seq.singleton (Array.map getHoledType signature, getHoledType ret)
    | Some pls ->
...
let rec private trySignature
    (inexactNum: int)
    (notPreferredNum: int)
    (compared: (SimpleValueType * SimpleValueType option) seq)
    : (int * int) option =
    match compared with
...
let private filterOverloadsByUnknowns
    (considered: (SimpleValueType[] * SimpleValueType)[])
    (args: SimpleValueType option[])
    =
    if Array.length considered = 1 then
        ...
        let allUnknowns =
            args
            |> Seq.mapiMaybe (fun i arg -> if Option.isNone arg then Some i else None)
            |> Seq.toArray
        ...
        if Array.isEmpty allUnknowns then
            ...
            let checkUnknownByOthers i =
                let stringSignatures =
                    considered
                    |> Seq.mapiMaybe (fun signI (sign, ret) ->
                        if sign.[i] = VTScalar STString then Some signI else None)
                    |> Set.ofSeq
                ...
                if not <| Set.isEmpty stringSignatures then
                    ...
                    | Some preferredSigns when Set.isEmpty preferredSigns ->
                        Set.ofSeq (seq { 0 .. considered.Length - 1 })
                    | Some preferredSigns -> preferredSigns

            let otherUnknownSets =
                allUnknowns |> Seq.map checkUnknownByOthers |> Seq.fold Set.intersect Set.empty
            ...
            let considered =
                if Set.isEmpty otherUnknownSets then
                    ...
                    considered
                    |> Seq.filteri (fun i _ -> Set.contains i otherUnknownSets)
                    |> Seq.toArray
            ...
            if Array.length considered = 1 then
                ...
                | Some firstTyp ->
                    if
                        ...
                        |> Seq.forall (function
                            | None -> true
                            | Some otherTyp -> otherTyp = firstTyp)
                        |> not
                    then
                        None
                    else
...
let findFunctionOverloads
    (signatures: FunctionSignaturesMap)
    (args: SimpleValueType option[])
    : (SimpleValueType[] * SimpleValueType) option =
    ...
    let sortedSignatures =
        expandedSignatures
        |> Seq.mapMaybe (fun (sign, ret) ->
            trySignature 0 0 (Seq.zip sign args)
            |> Option.map (fun key -> (key, (sign, ret))))
        |> Seq.sortBy fst
        |> Seq.toArray
    ...
    if Array.isEmpty sortedSignatures then
        ...
        if minErrors = 0 then
            ...
            let considered =
                sortedSignatures
                |> Seq.takeWhile (fun (key, pair) -> key = minKey)
                |> Seq.map snd
                |> Seq.toArray
...
let findBinaryOpOverloads
    (signatures: BinaryOperatorSignaturesMap)
    (a: SimpleValueType option)
    (b: SimpleValueType option)
    : ((SimpleValueType * SimpleValueType) * SimpleValueType) option =
    let convertBack (sign: SimpleValueType[], ret) = ((sign.[0], sign.[1]), ret)
    ...
    let sortedSignatures =
        expandedSignatures
        |> Seq.mapMaybe (fun (sign, ret) ->
            trySignature 0 0 (Seq.zip sign argsArray)
            |> Option.map (fun key -> (key, (sign, ret))))
        |> Seq.sortBy fst
        |> Seq.toArray
    ...
    if Array.isEmpty sortedSignatures then
        ...
        if minErrors = 0 then
            ...
            let checkFound =
                checkArgs
                |> Option.bind (fun cargs -> considered |> Seq.tryFind (fun (sign, ret) -> sign = cargs))
    ...
    if Array.isEmpty notUnknownArgs then
        ...
        let checkCandidateType candidateTyp argTyp =
            if (Map.find candidateTyp sqlReverseTypeCategories).IsPreferred then
                Error candidateTyp
            else if candidateTyp = argTyp then
                Ok candidateTyp
            else if
                tryImplicitValueCasts argTyp candidateTyp
                && not (tryImplicitValueCasts candidateTyp argTyp)
            then
                Ok argTyp
            else
                Ok candidateTyp
        ...
        let candidateTyp =
            Seq.foldResult checkCandidateType firstTyp notUnknownArgs |> Result.either id id
...
let valueSimpleType: Value -> SimpleValueType option =
    function
