...
type private PermissionsUpdater(db: SystemContext, allSchemas: Schema seq) as this =
    ...
    let updateAllowedFields (entityRef: ResolvedEntityRef) (entity: SourceAllowedEntity) (existingEntity: RoleEntity) =
        ...
        let createFunc (OzmaQLName fieldName) =
            RoleColumnField(ColumnName = fieldName, RoleEntity = existingEntity)

        ignore <| this.UpdateDifference updateFunc createFunc entity.Fields oldFieldsMap

    let updateAllowedEntity
        (entityRef: ResolvedEntityRef)
        (entity: SourceAllowedEntity)
        (existingEntity: RoleEntity)
        : unit =
    ...
    let updateAllowedDatabase (role: SourceRole) (existingRole: Role) : unit =
        let oldEntitiesMap =
            ...
            |> Seq.ofObj
            |> Seq.map (fun roleEntity ->
                ({ Schema = OzmaQLName roleEntity.Entity.Schema.Name
                   Name = OzmaQLName roleEntity.Entity.Name },
                 roleEntity))
            |> Map.ofSeq
        ...
        let entitiesMap =
            ...
            |> Map.toSeq
            |> Seq.collect (fun (schemaName, entities) ->
                entities.Entities
                |> Map.toSeq
                |> Seq.map (fun (entityName, entity) ->
                    ({ Schema = schemaName
                       Name = entityName },
                     entity)))
            |> Map.ofSeq
    ...
    let updatePermissionsSchema (schema: SourcePermissionsSchema) (existingSchema: Schema) : unit =
        let oldRolesMap =
            existingSchema.Roles
            |> Seq.map (fun role -> (OzmaQLName role.Name, role))
            |> Map.ofSeq
        ...
        let updateFunc name schema existingSchema =
            ...
            with e ->
                raisefWithInner SystemUpdaterException e "In allowed schema %O" name

        let createFunc (OzmaQLName name) =
            Role(Name = name, Schema = existingSchema)
...
type private RoleErrorRef =
    | ERRole of RoleRef
    | EREntity of AllowedEntityRef

let private findBrokenAllowedSchema
    (roleRef: RoleRef)
    (allowedSchemaName: SchemaName)
...
let private findBrokenRolesSchema (schemaName: SchemaName) (schema: PermissionsSchema) : RoleErrorRef seq =
    seq {
        for KeyValue(roleName, maybeRole) in schema.Roles do
            ...
            | Ok role -> yield! findBrokenRole ref role
            | Error e ->
    ...
    task {
        ...
        let roleChecks =
            ...
            |> Seq.mapMaybe (function
                | ERRole ref -> Some ref
                | _ -> None)
            |> Seq.map checkRoleName

        do! genericMarkBroken db.Roles roleChecks cancellationToken

        let allowedEntityChecks =
            broken
            |> Seq.mapMaybe (function
                | EREntity ref -> Some ref
                | _ -> None)
            |> Seq.map checkAllowedEntityName
