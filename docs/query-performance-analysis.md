...
### Правка кампаний — применена

Пять счётчиков сведены в один проход через `LEFT JOIN LATERAL` с `FILTER`. Применено к `marketing.campaigns_table` шестью точечными заменами через `safe_update_view_query`, каждая с проверкой перед коммитом.

**Корректность подтверждена:**

| кампания | было | стало |
|---|---|---|
| 2612 | 10 / 9 / 9 / 3 / 0 / 0 | 10 / 9 / 9 / 3 / 0 / 0 |
| 2611 | 2 / 1 / 1 / 1 / 0 / 0 | 2 / 1 / 1 / 1 / 0 / 0 |
| 2610 | 6 / 6 / 5 / 0 / 0 / 0 | 6 / 6 / 5 / 0 / 0 / 0 |
| 2609 | 6 / 6 / 6 / 2 / 0 / 0 | 6 / 6 / 6 / 2 / 0 / 0 |

Отдельно проверен `count(distinct ...) FILTER` на кампании с ненулевыми переходами: view отдаёт `79 / 78 / 29 / 4 / 0`, прямой запрос к базе — те же `79 | 78 | 29 | 4 | 0`. `FOR INSERT INTO` не сломался, страница работает, пагинация на месте.

**Выигрыша по времени страницы не видно.** До правки установившееся время было 353–456 мс, после — 405–545 мс с отдельными значениями 146–158 мс; разброс перекрывает возможный эффект. При этом основной запрос данных по замеру выполняется за 3–4 мс, то есть доминирующие ~400 мс уходят не в него. Разделить их точнее не вышло: серверные логи не размечены по HTTP-запросу, и на странице параллельно грузятся другие view.

Правка оставлена: она объективно сокращает число обращений к таблице на 1.34 млн строк (семь против трёх) и укорачивает SQL, но как ускорение конкретно этой страницы она себя не показала. Откат — заменой текста из [campaigns_table.optimized.funql](campaigns_table.optimized.funql) на оригинал.

Чтобы понять оставшиеся 400 мс, нужна инструментация по этапам запроса — те самые гистограммы на Prometheus, о которых шла речь в [performance-analysis.md](performance-analysis.md). Без них дальнейшая оптимизация этой страницы будет гаданием.

### Исходный вариант правки
